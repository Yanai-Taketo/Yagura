...
- リポジトリ変数(`vars`。公開してよい値):
  ...
  - `SIGNER_SUBJECT = 柳井建人`(署名者 Subject の期待値。訂正版証明書の実際の Subject は
    `C=JP, ST=Yamaguchi, O=柳井建人, CN=柳井建人`。CN も漢字表記であることに注意)
...
**未検証のまま本番リリースに使わないこと。** WIF 疎通(第 1 段)は 2026-07-18 に実証済み。
`codesign-chain.pem` は訂正版証明書(2026-07-23 再発行)受領後に追加済み。第 2 段(手順 7〜8)を
通してから本番リリースに使う。
