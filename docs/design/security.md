...
- **TLS ハンドシェイク失敗カウンタ**: `yagura.ingestion.tcp.tls_handshake_failure`（architecture.md §4.1.1。送信元アドレスをタグに持つ——本カウンタ体系で唯一のタグ付き計器。理由は同表参照）。`TlsSyslogListener` がハンドシェイク失敗（`AuthenticationException`/`IOException`/`OperationCanceledException`）のたびに計上する
- **イベント ID**: 起動時に証明書が解決できず縮小継続 = 1016、秘密鍵アクセス権付与 = 2010、期限接近の事前警告 = 1017、稼働中の使用不能 = 1018（§4.3 参照。いずれも additive-only で新設）
- **フォワーダキットの TLS 送信対応（同一 PR。送受信を揃えて実機検証を自己完結させる——オーナー決定 2026-07-10）**: 静的キット `forwarder/fluent-bit/install.ps1` に `-Mode tls`（既定ポート自動 6514・`-TlsCaFile`・`-TlsVerify`）を追加した。Fluent Bit の `out_syslog` に `Mode = tls` という値は無く（公式ドキュメント確認、2026-07-10）、`Mode tcp` + `tls On` の組み合わせで有効化する——`fluent-bit-yagura.conf` の `@@TLS_BLOCK@@` プレースホルダで注入する。管理 UI 生成キット（`/admin/forwarder-kit`）にも転送方式選択（UDP/TCP/TLS）を追加したが、**生成 UI は CA/サーバ証明書の同梱手段を持たない**（GET クエリでは複数行の PEM を安全に運べないため）——TLS 選択時は `tls.verify Off`（暗号化のみ・検証なし）で生成し、検証を要する場合は静的キットの `-TlsCaFile` へ誘導する。`ForwarderKitBuilder` 自体は CA 証明書同梱（`tls.verify On` + `tls.ca_file`）のロジックを実装・単体テスト済みであり、将来 UI 側を POST 化する等で経路を確保すれば有効化できる
- **既知の非互換（2026-07-11 実機検証で判明。重要）**: Fluent Bit 5.0.8 の `out_syslog` は TLS 有効時も RFC 6587 octet-counting フレーミングを実装しない（LF 区切りのまま。`-Mode tcp` の既存の既知の制約と同一原因）。Yagura の TLS 受信は RFC 5425 準拠で octet-counting を厳格に強制するため、**現時点の Fluent Bit は Yagura の TLS 受信へメッセージを配送できない**——TLS ハンドシェイクは成功するが、後続のメッセージは全てフレーミング違反として拒否される。`out_syslog` は送達確認を待たない実装のため、**Fluent Bit 側はこの拒否を検知できず、リトライもエラー報告も行わない**（ラボ検証: 2 通をそれぞれ別の keep-alive 接続で送信し、いずれも Yagura 側で拒否されたが Fluent Bit 側にはエラーが記録されなかった——送信側からは無音の喪失）。この事象は TLS ハンドシェイク失敗（本節上部のカウンタ）とは別経路で発生する——ハンドシェイク自体は成功するため当該カウンタは増加しない。詳細・回避策（rsyslog・syslog-ng 等 octet-counting 対応実装への案内）は `install.ps1` の `-Mode tls` コメントと [docs/guides/forward-windows-eventlog.md](../guides/forward-windows-eventlog.md) §「TLS を使う」に記録した。**この発見自体が、本 PR が「機能を実装しました」ではなく「実装して実機検証したら送信側の未解決の非互換が見つかった」という誠実な報告を要する理由である**——Yagura 側の TLS 受信実装自体（ハンドシェイク・証明書検証・octet-counting 強制・期限切れ時の継続動作）は openssl s_client による実機検証で正しく動作することを確認済み（[SEC-11](#7-確定待ち一覧)）
