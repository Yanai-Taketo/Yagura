...
- **Kerberos SPN の成立見通しと残る NTLM 経路（#228 実測で更新。[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 8）**: ADR-0010 起案時は「Kerberos SPN の仮想サービスアカウントでの成立可否は Phase 3 まで未検証」としていたが、**#228 の AD lab 実機検証（2026-07-11）で、仮想サービスアカウント構成のまま正規の SPNEGO-Kerberos・Kerberos AP-REQ が誤遮断されず疎通することを確認した**（§2.4 の Kerberos-only モード検証）。**この実測の直接の射程は「Kerberos-only フィルタが正規 Kerberos トークンを誤って NTLM として遮断しないこと」であり、Kerberos AP-REQ が実際に提示・受理される経路が存在した＝マシンアカウントの Kerberos が当該 lab で動作していたことを示す**。ここから**推論として**、Windows が `HOST/hostname` を機のコンピューターアカウントへ自動登録し `HTTP` サービスクラスがこの `HOST` SPN に包含される（Windows の標準挙動——**公式ドキュメントの引用・確認日は実装 PR で付す。本設計書は「そう見込む」に留め、断定しない**）ため、仮想サービスアカウントで動く管理リモート HTTPS へのホスト名アクセスでも Kerberos SSO が追加登録なしに成立すると**見込む**。**ただしブラウザからの管理 UI ログインが実際に Kerberos で成立し監査 2008（`scheme=windows`）に至る end-to-end の確認は #228 では取れておらず、SEC-14 (f)（ADR-0015 決定 7 (f)）の受け入れ条件として残る**——本節はこの E2E を「確定」とは書かない。以上を踏まえた見通し:
  - **ホスト名でのリモート管理アクセスは Kerberos SSO が成立する見込み**（SPN 未登録を前提にした NTLM 常用の想定は、少なくともホスト名経路については成り立たない見込み。E2E 確定は SEC-14 (f)）。**CNAME・別名・ロードバランサ経由の名でアクセスする構成は別 SPN 登録を要し得る**ため本見込みの対象外（その場合は該当名への SPN 登録手順が要る——operations ガイドの申し送り。SEC-D7 ①）
