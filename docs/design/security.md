...
### 1.1 閲覧層の外向きネットワーク通信の台帳（[ADR-0007](../adr/0007-reverse-dns-display.md) で新設。設計確定 2026-07-06・実装は後続 PR）
...
- 「オフ時にクエリを発しない」「対象帯域外の IP にクエリを発しない」は、解決 API の呼び出しを解決サービス 1 点に集約したうえで**単体テストとして固定する**（ADR-0007 決定 4。実装 PR の受け入れ条件）
