...
| SEC-6 | 失効済み circuit の閲覧継続の猶予の**上限値**（無期限を既定にしない）と猶予記録の詳細 | 認証 opt-in 実装時に確定（既定は「上限つき猶予 + 手動の即時全切断」）。**ADR-0010 Phase 4 補足（2026-07-12）**: 閲覧認証は circuit 認証状態の汲み直し（ADR-0010 決定 2。操作のたびに現在の認証状態で認可＝§2.3 の即時反映）を管理側と共有の `YaguraCircuitAuthenticationStateProvider` で既に受ける。本項が扱う「読み取り専用表示を失効の瞬間に切らない**猶予タイマー**」機構自体は未実装のバックログであり、その**上限値**の確定は当該タイマー実装時に引き受ける（値だけを先に凍結しない——掲示用途の実測を伴うため） | §2.3 |
...
| SEC-9 | ~~AD グループの指定形式（名前 / SID）とネストグループ解決の有無~~ → **ADR-0010 Phase 4（2026-07-12）で確定・実装済み**。名（`DOMAIN\Group`）と SID の両形式を受理し、起動時に SID へ解決してキャッシュ（解決不能な指定は警告してスキップ＝認可を付与しない安全側）。ネストは `WindowsIdentity.Groups`（推移的グループ SID・OS 展開済み）で解決し追加 LDAP 不要。照合はトークンのグループ SID 集合と設定 SID 集合の交差。役割: `Admin:...:AdminGroups`→管理（544 に加算）、`Viewer:...:ViewerGroups`→閲覧、`Viewer:...:AdminGroups`→管理（§3）。**AD 実環境（lab）での実機検証を受け入れ条件に含む** | §3 |
