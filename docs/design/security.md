...
| # | 通信 | 宛先 | 停止手段 | 根拠 |
|---|---|---|---|---|
| 1 | 送信元 IP の逆引き（PTR）DNS クエリ（対象はプライベート/サイトローカル系の帯域の IP のみ） | OS に構成済みの DNS サーバ（新しい宛先を設定させない） | `Viewer:ReverseDns:Enabled`（既定オン。オフでクエリ 0） | ADR-0007 |

- **台帳にない外向き通信を閲覧層に追加する PR は、本表の更新を同じ PR に含める**（L-5 の許可リストと同じ規約——「前例がある」で暗黙に増やさない）
