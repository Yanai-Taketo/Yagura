...
| 1009 | 1000 番台（運用警告） | エラー | スプールの定期自己検証に失敗 | スプールの定期自己検証（architecture.md §3.2.5。Issue #152）が失敗した——合成レコードの投入自体に失敗した（書込失敗・上限到達）、または投入した合成レコードが期待時間（仮値 10 分。architecture.md §9 M-16）以内に drain へ合流判定されなかった。障害時専用経路（スプール退避 → drain）の平常時検証が機能しておらず、実障害時の救済経路が働く保証を失っている状態のためレベルはエラー（本節のレベル割当方針「機能停止を伴う事象」）。`ActiveNotificationMonitor` による周期監視（Issue #152） |
