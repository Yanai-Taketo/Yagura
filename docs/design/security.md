...
以後、他の 3000 番台事象（認証失敗・失効猶予の記録等。§4.1 の対象一覧）を実装する際は 3003 以降を、2000 番台の管理操作は 2006 以降を、1000 番台の運用警告（自己検証の失敗——architecture.md §3.2.5・Issue #152、HTTPS 証明書の期限接近、circuit 上限の到達等。いずれも本 PR のスコープ外）を実装する際は 1009 以降を additive-only で追加し、本表を更新する。
