...
- **証明書の参照方式（決定 4）**: Windows 証明書ストア（ローカルコンピューター・`My`）からの拇印参照——configuration.md §6（閲覧 UI の HTTPS）と同型の方式を採用する（`Yagura.Host.Administration.Https.CertificateProvider`）。設定キーは閲覧 UI の HTTPS とは独立（`Admin:Https:*`。同一証明書の流用は両方のキーに指定することで実現し、暗黙の連動はしない）
- **秘密鍵アクセス権の付与**: 証明書が解決できた場合、サービスアカウント（`NT SERVICE\Yagura`）へ秘密鍵の読み取り権限のみを自動付与する（`Yagura.Host.Administration.Https.CertificatePrivateKeyAccessGranter`）。**対応範囲は CNG（ソフトウェアキーストレージプロバイダー）ベースの秘密鍵に限る**——鍵コンテナファイル（`%ProgramData%\Microsoft\Crypto\Keys\<UniqueName>`）への ACE 追加という実装方式上、スマートカード・HSM・TPM 保護鍵等のファイルとして存在しない秘密鍵には対応できない（その場合は証明書スナップイン `certlm.msc` からの手動権限付与に誘導するエラーメッセージを返す——configuration.md §6 CF-D2 と同じフォールバック）。付与に成功した場合は監査記録の対象（イベント ID 2009。`AuditEventKind.AdminHttpsCertificatePrivateKeyAccessGranted`）。付与に失敗しても起動は妨げない（現在の実行アカウントが既に証明書へアクセスできる場合は動作を継続できるため——警告のみ）
...
**③ 権限が全く無い状態でも起動は継続する（発見時は起動失敗の不具合。Issue #345 で 2026-07-19 修正済み）**。修正前は、上記の警告に到達する前に鍵ファイルのパスを解決する処理が未処理例外となりプロセスが落ちていた:

```
System.Security.Cryptography.CryptographicException: キー セットがありません。
   at System.Security.Cryptography.CngKey.Open(String keyName, CngProvider provider, CngKeyOpenOptions openOptions)
   at Yagura.Host.Administration.Https.CertificatePrivateKeyAccessGranter.ResolveCngKeyFilePath(X509Certificate2 certificate)
```

`ResolveCngKeyFilePath` は**付与先のファイルパスを知るために秘密鍵を開く**が、その open 自体が付与対象の権限を要する（鶏と卵）。結果として「サービスアカウントに権限が無い」という本機構が想定する典型状況で、警告ではなく**起動失敗**になっていた（SCM からは 7000/7009——時間内に応答しない——として観測）。開発機の既存回帰テスト（`AdminRemoteBindingRegressionTests`）が通っていたのは、テストが管理者権限のプロセスで走り鍵を開けるためであり、**SEC-13 ① が「未確認」として挙げていた差分そのもの**である。現在は `TryGrantReadAccess` がこの `CryptographicException` を捕捉し、certlm.msc（CF-D2）への手動付与誘導を含む警告として返す——「付与に失敗しても起動は妨げない（警告のみ）」が権限ゼロの典型状況でも成立する（呼び出し元は管理 HTTPS と TLS 受信の 2 箇所で、どちらも同じ経路で救済される）。回帰検証は CI では原理的に成立しない（テストが管理者権限で走り鍵を開けられる）ため、lab 手順書（`installer/lab/startup-warning-regression-procedure.md`。#356）に固定した。
...
- **証明書の期限接近・稼働中異常の周期監視（決定 4「期限接近の事前警告」の実体。PR #224 レビュー指摘 #2・#3 への対応）**: `ActiveNotificationMonitor`（architecture.md §4.6 の能動通知。1 分周期・抑制窓 15 分）に管理リスナのリモート HTTPS 証明書の評価を追加した（`ICertificateStatusProbe` / `StoreAdminHttpsCertificateStatusProbe`——周期ごとに証明書ストアを拇印で再照会する）。①有効期限が閾値（仮値 **30 日**。`ActiveNotificationConstants.CertificateExpiryWarningWindow`）以内に接近したら **1014** で事前警告、②稼働中にストアから削除された・秘密鍵にアクセスできなくなった・期限切れへ遷移した（= `ServerCertificateSelector` が新規ハンドシェイクを拒否している状態）ら **1015** で継続警告する（§4.3）。プローブの結線はリモート HTTPS bind が実際に有効な場合のみ——起動時に証明書を解決できず縮小継続した構成は 1013 が報告済みで、再起動なしに bind が有効化されることもないため周期監視の対象外。**限界の明示**: 個々の TLS ハンドシェイク失敗イベントを Kestrel から拾う配線は持たない（`ServerCertificateSelector` の `null` 返却はハンドシェイク失敗として TLS レベルで処理され、アプリ層のフックが公式に提供されていない）——稼働中の証明書異常の検知は本周期監視（状態の検知。最大 1 分の遅延）が受け持つ
...
- **Kerberos SPN の成立見通しと残る NTLM 経路（#228 実測で更新。[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 8）**: ADR-0010 起案時は「Kerberos SPN の仮想サービスアカウントでの成立可否は Phase 3 まで未検証」としていたが、**#228 の AD lab 実機検証（2026-07-11）で、仮想サービスアカウント構成のまま正規の SPNEGO-Kerberos・Kerberos AP-REQ が誤遮断されず疎通することを確認した**（§2.4 の Kerberos-only モード検証）。**この実測の直接の射程は「Kerberos-only フィルタが正規 Kerberos トークンを誤って NTLM として遮断しないこと」であり、Kerberos AP-REQ が実際に提示・受理される経路が存在した＝マシンアカウントの Kerberos が当該 lab で動作していたことを示す**。ここから**推論として**、`HTTP` サービスクラスがコンピューターアカウントの `HOST/hostname` SPN に包含されれば（Windows の SPN マッピング〔`sPNMappings`〕に依存する挙動で、**包含の成立は自明ではない**——バージョン・構成で異なり得る）、仮想サービスアカウントで動く管理リモート HTTPS へのホスト名アクセスでも Kerberos SSO が追加登録なしに成立し得る。**ただし、この `HTTP`⊂`HOST` の機序そのものと、ブラウザからの管理 UI ログインが実際に Kerberos で成立し監査 2008（`scheme=windows`）に至る end-to-end の両方が #228 では確認できておらず、あわせて ADR-0010 委任事項 3（Phase 3 の実機確認。公式ドキュメントの照会と確認日の記録を含む）で検証する**——本節はこの成立を「確定」とも「前提」とも書かない（gMSA の受け入れ条件 SEC-14 (f) は別事項＝gMSA への HTTP SPN 登録手順の確定であり、仮想 SA のこの E2E の受け皿ではない）。以上を踏まえた見通し:
  - **ホスト名でのリモート管理アクセスは Kerberos SSO が成立する見込み**（SPN 未登録を前提にした NTLM 常用の想定は、少なくともホスト名経路については成り立たない見込み。**機序・E2E の確定は ADR-0010 委任事項 3 / Phase 3**）。**CNAME・別名・ロードバランサ経由の名でアクセスする構成は別 SPN 登録を要し得る**ため本見込みの対象外（その場合は該当名への SPN 登録手順が要る——operations ガイドの申し送り。SEC-D7 ①）
  - **IP アドレスでのアクセスは依然 NTLM 経路**（IP はホスト名ベースの SPN 一致対象を持たないため。ADR-0010 検証 4 の構造的縮退。非ドメイン参加機・ワークグループ構成のローカルアカウントも同様に NTLM）——この残る NTLM 経路の既知の弱点（リレー攻撃・pass-the-hash・相互認証の欠如）を受容する。緩和策は Kerberos-only モード（決定 2。§2.4——有効化すると IP アクセスの NTLM は 403 で拒否され、ホスト名 + Kerberos のみになる）と HTTPS 必須（本節）
  - **SPN 起因での gMSA 要求は消えた**: ADR-0010 委任事項 3 の「仮想サービスアカウントへの SPN 登録が成立しない場合、gMSA との接続で再評価する」は、（上記の見込みの範囲で）**決着**した（gMSA が SPN のために必要になることはない）。gMSA は別の理由——リモート SQL Server への DB 側最小権限付与・保存/監査の主体をドメイン管理体系に載せること——で opt-in 提供する（ADR-0015。§5.2・database.md §6.1）
  - **逆に、DB を動機に gMSA へ切り替えると既存のホスト名 Kerberos SSO が退行し得る（ADR-0015 決定 7 (f)）**: gMSA はコンピューターアカウントとは別のドメインアカウントであり、`HOST/hostname` の自動登録に与らない。リモート管理 HTTPS をホスト名 + Kerberos で使っている構成が gMSA へ切り替わると、**gMSA へ `HTTP/hostname` SPN を明示登録しない限り Kerberos SSO が NTLM へ後退する**。gMSA 採用時の HTTP SPN 登録手順を operations ガイドの成果物として確定する（SEC-D8・SEC-14 (f)）——「gMSA にすれば良くなる」だけを書いて Kerberos 面の退行を伏せない
  - 利用者向けドキュメントへの明記は §8 SEC-D7
...
- **他文書が「監査記録に乗せる」と定めた項目の受け皿**（逆引き。網羅性の検証点）:
  ...
  - **サービス実行アカウントの構成と変化の記録**（[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 8。configuration.md §4.4）: `YAGURA_SERVICE_ACCOUNT` の構成値（既定の仮想サービスアカウント / gMSA）を初回起動時にイベントログへ転記する（2024。§4.3。インストーラ由来転記レールに合流し、2000 番台 additive-only）。さらに**起動時に実効実行アカウント（プロセスが実際に動いている識別）を前回起動時の記録と照合し、変化を監査事象として記録する**（2025）——製品外の `sc config` による切替（ADR-0015 決定 6 の稼働中切替手順）も次回起動で必ず証跡化され、「いつからこの識別で動いているか」に監査記録で答えられる。前回記録の永続先はデータルート直下の専用ファイル（`last-service-account.json`。FW 転記マーカー・`last-applied-configuration.json` と同じく書き込み主体を専用ファイルへ分離する——設定差分照合（2019）と同じ「変化検出」レール）。**このスナップショットの改竄耐性は `last-applied-configuration.json` と同水準にとどまる**——サービスアカウント書き込み可（実行アカウントを切り替えられる者＝管理者は消せる）であり、悪意への統制ではなく事故調査のための運用証跡である。一次の耐タンパ線は 2024/2025 の**イベントログ併記**（消去が痕跡を残す。§4.2）。初回起動・記録欠損時は転記（2024）のみ行い変化記録（2025）はスキップする
...
| 1014 | 1000 番台（運用警告） | 警告 | 対象（警告） | 管理 UI リモート HTTPS 証明書の期限が接近 | 管理リスナのリモート HTTPS 証明書の有効期限が閾値（仮値 30 日。`ActiveNotificationConstants.CertificateExpiryWarningWindow`）以内に接近した（ADR-0010 Phase 2 決定 4 の「期限接近の事前警告」の実体。§2.5）。`ActiveNotificationMonitor` の周期監視（1 分間隔・抑制窓 15 分） |
...
| 1017 | 1000 番台（運用警告） | 警告 | 対象（警告） | TLS 受信証明書の期限が接近 | TLS 受信証明書の有効期限が閾値（仮値 30 日。管理 UI HTTPS 証明書（1014）と同じ `CertificateExpiryWarningWindow` を流用）以内に接近した（§6）。`ActiveNotificationMonitor` の周期監視（1 分間隔・抑制窓 15 分） |
...
| 3012 | 3000 番台（拒否・セキュリティ事象） | 警告 | — | 拒否試行を集約記録 | 同一送信元・同一種別の拒否が短時間に反復した場合の集約サマリ（§4.4 SEC-4。Issue #268）。集約対象の個別事象（3002/3003/3004/3006/3008/3009/3014）が閾値・窓を満たしたときに個別記録から切り替わり、静穏窓の経過で 1 件記録される。記録内容は集約対象種別・回数（全件——集約中も件数を失わない）・期間・試行された利用者名の集合（§4.4「どのアカウントが狙われたか」の次元）・最初と最後の試行のフル詳細 |
...
| 1028 | 1000 番台（運用警告） | 警告 | 対象（警告） | 複数の送信元が一斉に途絶 | 同一評価周期に閾値件数（仮値 5 件。CF-9）以上が途絶へ遷移した場合、個別警告ではなく 1 件の集約警告にする（ADR-0018 決定 3）。集約スイッチ障害等で 50 台が同時に黙ったとき、個別 50 件（メール通知有効時は 50 通）は診断情報としても劣化しているため。Detail には件数・エントリ一覧（列挙は上限 20 件 + 超過分の総数表記——Windows イベントログのメッセージ長上限〔約 31KB〕を考慮。全量はダッシュボードの途絶中強調で確認する。Issue #382）に加え、「サーバ側受信経路の状態を先に確認」する誘導と、**起動後の再アーム起点の一斉発火かの別**を含める（同一閾値で再アームされたエントリ群は起動から閾値経過後に同時発火し得る——独立障害の寄せ集めを「共通上流障害」と誤誘導しない）。**集約時も各エントリの途絶フラグ・抑制窓は個別警告時と同様に更新する** |
...
| 1030 | 1000 番台（運用警告） | エラー | 対象（エラー） | 恒久障害によりバッチ書き込みが失敗 | DB へのバッチ書き込みが恒久障害（database.md §1.2 契約 3 の分類——設定・スキーマ・権限など再試行で解消しない失敗）で失敗し、スプールへの退避を開始した（[ADR-0017](../adr/0017-email-notification.md) 委任 10 の裁定。Issue #369）。発火点は `PersistenceWriter`（抑制窓付き——同種の警告は仮値 5 分の間は再表示しない）。恒久障害の開始は「放置すればスプールが満ちる」導火線の点火であり、1004（退避継続）経由の間接検知は継続判定（仮値 5 分）の後にしか出ず根本原因も示さないため、採番して直接通知の対象にする。従来は EventId なし（= 0）で書き出されておりメール通知が構造的に捕捉できなかった |
| 1031 | 1000 番台（運用警告） | エラー | 対象（エラー） | Windows 統合認証での DB 接続失敗 | 稼働中の Windows 統合認証での SQL Server 接続失敗によりバッチ書き込みが恒久障害となり、スプールへの退避を開始した（[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 5 の観測性要件。database.md §6.1。Issue #418）。イベントログ本文に**実行主体（実効実行アカウント名）と失敗種別**——DC 起因（SSPI コンテキスト生成失敗 = DC 到達不能・SPN 解決不能等）か SQL Server 起因（ログイン未作成 18456 / DB 不在・接続権限不足 4060）——を含め、一次切り分けがサーバ単体でつくようにする。分類根拠は SEC-14 (a)/(c) の AD lab 実測（2026-07-24。[ADR-0015](../adr/0015-gmsa-service-account.md) 改訂履歴 3——推測の分類表を作らない）。発火点は `PersistenceWriter`——1030 と同一の発火点・抑制窓（仮値 5 分）を共有し、統合認証の接続失敗と分類できた恒久障害では 1030 の**代わりに**本 ID を出す（同一例外で二重警告にしない。分類できない失敗は従来どおり 1030）。メール通知対象（エラー）とするのは、1030 の代替として発火する以上、同じ重大度で登録しないと gMSA 構成の夜間 DC 断——「書き込みが止まる」導火線（database.md §6.1 の名指し）——の夜間到達が退行するため |
...
| 2024 | 2000 番台（管理操作の監査） | 情報 | — | サービス実行アカウントの構成を転記 | サービス実行アカウント（`YAGURA_SERVICE_ACCOUNT`。既定の仮想サービスアカウント `NT SERVICE\Yagura` / gMSA `DOMAIN\name$`）の構成値の初回起動時のイベントログ転記（[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 8。§4.1・configuration.md §4.4）。インストーラ由来転記レール（2017 と同型）に合流する。記録内容は構成された実行アカウント名（gMSA 名は識別子であり秘密ではない）。転記は 1 回のみ（マーカーで判定） |
| 2025 | 2000 番台（管理操作の監査） | 情報 | — | サービス実行アカウントが前回起動時から変化 | 起動時に実効実行アカウントが前回起動時の記録（`last-service-account.json`）と異なる場合に 1 件記録する（ADR-0015 決定 8。§4.1）。製品外の `sc config` による切替（決定 6 の稼働中切替手順）も含めて次回起動で必ず証跡化する。記録内容は旧・新の実行アカウント名——「いつからこの識別で動いているか」に監査で答える。初回起動・記録欠損時は発火しない（転記 2024 のみ）。2019（設定ファイルの変化検出）と同じ「変化検出」レールだが、対象が実行アカウントの識別である点が異なる。**メール通知は対象外（—）**——2000 番台の管理操作監査（情報レベル）は一律メール非対象で、夜間到達が要る運用警告（1000 番台）とは区画が異なる（§4.3 のレベル割当方針）。AD 側の取得許可の事後変化による起動失敗は 1069 系（決定 3 の 3 段チェックリスト）が別途扱う（帰結 3） |
...
| 1032 | 1000 番台（運用警告） | エラー | — | フォワーダ MSI アップロードの fail-closed 設定を拒否して起動を中止 | `Admin:ForwarderKit:MsiUpload:Enabled` が有効なのに前提条件（管理 UI 認証のいずれか + `Admin:Authentication:RequireForLoopback`）が満たされていない設定の起動時拒否（[ADR-0020](../adr/0020-forwarder-msi-upload.md) 決定 1・委任 1。§5.1。Issue #283）。1011/1012 と同型。エラーメッセージに復旧に必要な具体の設定キーと値（`Admin:ForwarderKit:MsiUpload:Enabled を false に`）を含める——手編集復旧の場面では UI の誘導が使えないため |
| 1033 | 1000 番台（運用警告） | 警告 | — | フォワーダ MSI 配置フォルダの ACL 乖離 | アップロード機能が無効なのに、配置フォルダにサービス実行アカウントの書き込み ACE が残っている（閉じ忘れ。ADR-0020 決定 2・委任 7。§5.1。#171 の教訓——意図した ACL と実 ACL の乖離は検出されるまで気づかれない）。起動時の一回検査と周期監視（`ActiveNotificationMonitor`。ACL の読み取りのみで判定——実書き込みプローブは SACL 監査を毎分汚染するため採らない）の両方から発火する |
| 1034 | 1000 番台（運用警告） | **情報** | — | フォワーダ MSI の書き込み経路の開放が継続 | アップロード機能が有効で、書き込み ACE の存在が継続判定期間（仮値 24 時間）以上連続して観測された（ADR-0020 決定 2・委任 7。§5.1）。設定上は期待どおりの状態（常置運用は正当な選択）であり、「開いたままである」ことの定期リマインダとして専用の長い抑制窓（仮値 7 日）で再表示する。「使うときだけ開く」運用の閉じ忘れの主たる検出手段。1000 番台の情報レベルは 1029 に次ぐ 2 例目 |
| 2026 | 2000 番台（管理操作の監査） | 情報 | — | フォワーダ MSI を管理画面から配置 | フォワーダ MSI の管理画面アップロードの配置確定（ADR-0020 決定 3・4。配置経路 (b)。Issue #283）。記録内容は操作者（認証方式 + 認証済み利用者名——前提条件により必ず値が入る）・接続元・loopback/リモートの別・アーキテクチャ・格納ファイル名・ProductVersion・SHA256・サイズ・officialHashMatch・versionMismatchAcknowledged・置換時の旧 SHA256 |
| 2027 | 2000 番台（管理操作の監査） | 情報 | — | フォワーダ MSI を管理画面から削除 | 配置済みフォワーダ MSI の管理画面からの削除（ADR-0020 決定 3・4）。削除は配置と同じ重みの操作として扱い、削除前の SHA256 を記録する。操作者の記録は 2026 と同じ |
| 3014 | 3000 番台（拒否・セキュリティ事象） | 警告 | — | フォワーダ MSI のアップロード/削除を拒否 | フォワーダ MSI のアップロード・削除の失敗/拒否（ADR-0020 決定 4。ADR-0004 決定 7「拒否された試行も監査対象」）。検証失敗（サイズ超過・ProductVersion 読み取り失敗・多重状態）・二段階確認の拒否（利用者の中止）・書き込み失敗・排他拒否・確認不整合（TOCTOU ガード）を `Detail` の理由種別（`reason=`）で区別する。§4.4 の集約対象 |
| 1035 | 1000 番台（運用警告） | 警告 | — | 閲覧 UI の HTTPS が成立せず閲覧リスナを開かずに縮小継続 | 閲覧 HTTPS（`Viewer:Https:Enabled`。[ADR-0022](../adr/0022-viewer-https.md)。opt-in）が構成されているのに、証明書ストア参照の失敗（証明書が見つからない・秘密鍵にアクセスできない・既に期限切れ）または設定の静的な不成立（拇印未設定・形式不正等——`ViewerHttpsMode.SuppressListener`）により、閲覧リスナ（既定 8514）を開かずに縮小継続した（決定 2。**平文 HTTP では開かない**）。受信・保存・管理リスナは影響を受けず、閲覧系ルートは管理リスナ（loopback）経由で引き続き到達できるため、1013/1016 と同区分の「警告」とする。メール通知は本 ID（起動時 1 回）ではなく稼働中の周期監視 ID（1018 の閲覧版。実装は通知段——ADR-0022 決定 2 可視化①）が担う |
| 2028 | 2000 番台（管理操作の監査） | 情報 | — | 閲覧 UI HTTPS 証明書の秘密鍵アクセス権を付与 | 閲覧 HTTPS 証明書の秘密鍵読み取り権限をサービスアカウントへ付与した（[ADR-0022](../adr/0022-viewer-https.md) 決定 3。configuration.md §6「付与は監査記録の対象とする」）。記録内容は証明書拇印・付与先アカウント名。秘密鍵そのものは記録しない。2009/2010 と同型（起動時の自動操作） |

アプリ独自認証のバックオフ cap 到達・IP レート制限/グローバルトークンバケット拒否は [ADR-0011](../adr/0011-app-auth-failure-backoff.md) 決定 9 で 3006・3007 として実装済み。アプリ独自認証の三層防御の能動通知への昇格は 1019（`AdminAuthFailureDefenseEscalated`）として実装済み（決定 6）。失効後の閲覧継続の猶予・猶予終了は SEC-6 で 3010・3011 として（Issue #267）、拒否試行の集約記録は §4.4 SEC-4 で 3012 として（Issue #268）、監査チャネル復旧・書き戻し完了は §4.2 SEC-10 で 3013 として（Issue #269）実装済み。フォワーダ MSI のアップロード/削除の拒否は [ADR-0020](../adr/0020-forwarder-msi-upload.md) 決定 4（Issue #283）で 3014 として実装済み。以後、他の 3000 番台事象を実装する際は 3015 以降を（3008 = ADR-0010 の認証成功後の認可拒否、3009 = ADR-0010 Phase 4 の閲覧認証成功後の認可拒否、3010・3011 = SEC-6 の失効猶予、3012 = SEC-4 の拒否試行集約、3013 = SEC-10 の監査チャネル復旧、3014 = ADR-0020 の MSI アップロード拒否として実装済み）、2000 番台の管理操作（管理リスナのリモートバインド設定変更・リモート HTTPS 証明書設定変更は [ADR-0012](../adr/0012-admin-https-cert-ui.md) 決定 7 で 2011・2012 として、認証セッションの緊急全失効は [ADR-0013](../adr/0013-admin-winauth-session.md) 決定 2 で 2013 として、閲覧 UI へのサインイン成功は ADR-0010 Phase 4 決定 7 で 2014 として、監査記録の保持期間削除は SEC-2 で 2015 として、設定再読み込みは CF-4 で 2016 として、インストール記録の転記は CF-2 で 2017 として、蓄積ログ移行は DB-5 で 2018 として、起動時の設定差分照合は Issue #329 で 2019 として、TLS 受信の証明書設定変更は [ADR-0019](../adr/0019-ingestion-tls-cert-ui.md) 決定 5 で 2020 として、メール通知の設定変更・テスト送信は [ADR-0017](../adr/0017-email-notification.md) で 2021・2022 として、途絶検知のウォッチリスト変更は [ADR-0018](../adr/0018-source-silence-detection.md) で 2023 として、サービス実行アカウントの構成転記・前回起動時からの変化検出は [ADR-0015](../adr/0015-gmsa-service-account.md) 決定 8 で 2024・2025 として、フォワーダ MSI の配置・削除は [ADR-0020](../adr/0020-forwarder-msi-upload.md) 決定 4 で 2026・2027 として、閲覧 UI HTTPS 証明書の秘密鍵アクセス権付与は [ADR-0022](../adr/0022-viewer-https.md) 決定 3（Issue #455）で 2028 として実装済み）を実装する際は 2029 以降を、1000 番台の運用警告（circuit 上限の到達等。自己検証の失敗は Issue #152 で 1009 として、そのバックログ起因の区別は PR #211 で 1010 として、管理 UI 認証の fail-closed 拒否は ADR-0010 Phase 1 で 1011 として、管理リスナのリモートバインドの fail-closed 拒否・証明書解決失敗・証明書期限接近・稼働中の証明書使用不能は ADR-0010 Phase 2 で 1012〜1015 として、TLS 受信証明書の起動時解決失敗・秘密鍵アクセス権付与・期限接近・稼働中使用不能は Issue #137 で 1016〜1018・2010 として、アプリ独自認証の三層防御の能動通知への昇格は ADR-0011 で 1019 として、再読み込みの未反映残存・拒否・縮小継続・FW 規則不一致は Issue #262/#291/#265 で 1020〜1023 として、設定読み取り失敗による起動失敗は Issue #312 で 1024 として、メール通知の送信失敗・流量制御による抑制は [ADR-0017](../adr/0017-email-notification.md) で 1025・1026 として、送信元の途絶検知・一斉集約・復帰は [ADR-0018](../adr/0018-source-silence-detection.md) で 1027〜1029 として、恒久障害による書き込み失敗は ADR-0017 委任 10（Issue #369）で 1030 として、稼働中の Windows 統合認証での DB 接続失敗は [ADR-0015](../adr/0015-gmsa-service-account.md) 決定 5（Issue #418）で 1031 として、フォワーダ MSI アップロードの fail-closed 拒否・ACL 乖離・開放継続は [ADR-0020](../adr/0020-forwarder-msi-upload.md) 決定 1〜2（Issue #283）で 1032〜1034 として、閲覧 UI HTTPS の起動時証明書解決失敗による縮小継続は [ADR-0022](../adr/0022-viewer-https.md) 決定 2（Issue #455）で 1035 として実装済み）を実装する際は 1036 以降を additive-only で追加し、本表を更新する。

**「メール通知」列について（2026-07-19。[ADR-0017](../adr/0017-email-notification.md) 決定 6）**: メール通知（opt-in）の対象となるイベント ID を本表で定義する。**既定は「対象外」（`—`）であり、新しい ID を追加する PR は本列を必ず埋める**——「メールに乗せるか」を明示的に判断させるための更新義務である。`対象（警告）` / `対象（エラー）` の別は流量制御の挙動を決める（エラーは全体流量上限〔仮値 10 通/時〕の対象外。ただし ID 粒度の再送間隔〔仮値 60 分〕は等しく適用されるため無制限ではない）。この重大度は**本表が正本**であり、発火点で使われている `LogLevel` からは読み取らない——発火点のログレベルを変える PR が流量制御の挙動を黙って変えることを避けるため。実装は `EmailNotificationAllowlist`（`Yagura.Host.Observability.ActiveNotification.Email`）で、**本表の「メール通知」列と allowlist の一致は `EmailNotificationAllowlistSyncTests` が機械検証する**（表を正本とし、対象/対象外・重大度の食い違いで CI が落ちる。あわせて通知系発火点に EventId 未指定の警告・エラーログが増えていないか——`[permanent-failure]`＝1030 と同じ欠陥クラス——を検出する。ADR-0017 委任 6。Issue #387）。`対象外（構造的）` はメール通知チャネル自身が発する ID（1025・1026）につく印で、**送信失敗をメールで通知しようとするループを定義レベルで排除している**ことを示す（allowlist に載せた瞬間にテストが落ちる）。
...
- **サービス動作への影響なし（実機確認済み・2026-07-09）**: `SystemForwarderMsiSource`（`src/Yagura.Web/ForwarderKit/SystemForwarderMsiSource.cs`）はフォルダの列挙・ファイルの SHA256 計算・MSI 版取得（`MsiOpenDatabase` 読み取り専用モード + Property テーブル参照。Issue #436 で `MsiGetFileVersion` から変更——読み取り専用である性質は変わらない）のみを行う読み取り専用の実装であり、書き込みは一切行わない。実機で MSI インストール → `Administrators` 権限でファイル配置 → `NT SERVICE\Yagura` で稼働する Yagura.Host の `/admin/forwarder-kit` へアクセスし、配置した MSI が「未検出」にならず正しく検出されることを確認した
...
#### 5.1.1 配置経路 (b): 管理画面アップロード（[ADR-0020](../adr/0020-forwarder-msi-upload.md)。opt-in。Issue #283）

上記の手動配置（経路 (a)）に加え、**管理画面からのアップロード（経路 (b)）を認証有効時限定の opt-in で提供する**。前提・統制の全設計は ADR-0020 のとおり。実装の要点:

- **有効化条件（決定 1。[ADR-0021](../adr/0021-msi-upload-operation-auth.md) 決定 2 により改訂——`RequireForLoopback` 必須は撤廃）**: `Admin:ForwarderKit:MsiUpload:Enabled = true` は、管理 UI 認証（§2.4）のいずれかが有効（= サインインの手段が存在する）な構成でのみ受理される。満たさない組み合わせは起動時 fail-closed（イベント ID 1032。`YaguraConfigurationLoader.Load`——1011/1012 と同型。判定条件のみ ADR-0021 に追随し ID の意味は不変）。**lab 観測の留意（決定 5 lab P-1。2026-07-25。[Issue #433](https://github.com/Yanai-Taketo/Yagura/issues/433)）**: この fail-closed は Host 構築前の bootstrap ロガー（`Program.cs` の `AddConsole()` 専用）で `LogCritical(1032)` したのち `ConfigurationValidationException` を re-throw する。**Windows サービスとして動かすと 1032 は Windows イベントログに現れず**、管理者がイベントログで見るのは `.NET Runtime` 1026 + Application Error 1000（例外本文に欠落条件・誘導文言は入る）になる——これは 1032 固有でなく Host 構築前の全 config 検証 fail-closed（1011/1012/1024/1032）に共通。サービス構成での観測性の是正は #433 で追跡。UI 側（生成画面）でも同じ条件を先に判定し、不成立時は機能を表出させず「どの条件が欠けているか + 手動配置ガイドへの導線」を案内する。**アップロード関連エンドポイントは条件成立時のみ登録する**（構造的非存在——`MapYaguraAdmin` の条件分岐。無効構成では 404）
- **アップロード操作単位の実認証（ADR-0021 決定 1）**: 無認証 loopback からの到達遮断は、リスナ全体の認証必須（`RequireForLoopback`）ではなく**アップロード系操作専用の認可ポリシー**（`AdminAuthenticationExtensions.ForwarderMsiUploadPolicyName`。暗黙 loopback バイパス条項を持たず、実際にサインインした管理セッション——`IsAdminSessionAuthenticated`——のみ許可。閲覧セッションは不可）が担う。対象は stage・antiforgery トークン配布（HTTP。宣言的 `RequireAuthorization`）と commit/discard/delete（circuit。`ForwarderMsiPlacementService` が同一判定 `IsForwarderMsiUploadOperationAllowed` で最終防衛——UI の出し分けも `ForwarderKitScreen` が同じ関数で行う）。**認可拒否は監査 3014（`reason=operation-auth-required`）に記録する**（HTTP 側は `ForwarderMsiUploadAuthorizationAuditHandler`——`IAuthorizationMiddlewareResultHandler` のラップで宣言的認可と拒否監査を両立。「拒否されること」だけでなく「拒否が見えること」が設計条件）。loopback の無認証閲覧・通常管理は従来どおり（ゼロ設定思想を維持。`RequireForLoopback` 自体の機能は ADR-0010 決定 1 のまま不変）
- **資格情報発行口の統制（ADR-0021 決定 1）**: アップロード機能が有効な稼働構成では、認証設定の変更・アプリアカウントの作成/変更（`AdminAuthenticationAdminService.ConfigureAsync`）にも同じ実認証判定を要求する——無認証 loopback でアカウントを自己発行 → その資格情報でサインイン → 専用ポリシー通過、というブートストラップ迂回を閉じる。機能無効の構成では従来どおり（ADR-0010 決定 3 の bootstrap 設計は不変）。あわせて、(iii) が設定ファイル上有効なまま認証方式を両方無効化する変更は UI レベルで拒否する（1032 の対称防御——1012 型の UI 二段構えと同型）。(iii) 有効化前に仕込まれた資格情報への**切替時点検は実装済み**（ADR-0021 委任 2）: 生成画面の opt-in トグル（`IForwarderMsiUploadAdminService`。トグル操作の認可もアップロード操作と同格＝実サインイン必須）が、有効化時に既存アプリアカウントの**ユーザー名・作成時刻・最終変更時刻**を提示して管理者の確認を求める（未確認は書き込み前に拒否）。提示した内容と確認の有無は監査 2001 の Detail に残す。時刻の保持にはアカウントストアのスキーマ v3（`CreatedAtUtc`/`UpdatedAtUtc`。SQLite・SQL Server 両 provider）を追加した——**v3 より前に作成された既存行は NULL = 「不明」と表示する**（移行時刻でのバックフィルは「移行日に作られた」という嘘になるため行わない）
- **既定 ACL は不変・開放は OS 管理者の明示操作のみ（決定 2）**: 本節冒頭の ACE 構成（サービスアカウント読み取りのみ）は経路 (b) の実装後も**既定のまま変わらない**。Yagura 自身は ACL を変更せず（サービスアカウントは WRITE_DAC を持たない——構造的に不可能）、インストーラによる付与も無条件・opt-in を問わず提供しない。書き込みが成立するのは、管理者が `icacls` で実効実行アカウント（仮想 SA / gMSA——§5.2 の導出原則）に本フォルダ限定の Modify ACE を明示付与した場合のみ。付与コマンド・撤去コマンド・SACL（OS 監査）の推奨設定は利用者ガイドが同一手順書内で案内する
- **書き込み実装（決定 3）**: `SystemForwarderMsiStore`（`src/Yagura.Web/ForwarderKit/SystemForwarderMsiStore.cs`）。アップロード本文は配置フォルダ内のステージング名（`.uploading-<GUID>.msi`——検出パターン非一致のため `Lookup()`・生成処理から不可視）へ直接ストリーム書き込みし、検証（ProductVersion 読み取り〔クライアント申告のファイル名は保存名に使わない——版の根拠は ProductVersion のみ〕・SHA256・公式ハッシュ照合・既存ファイルとの関係）を通過した確認後にのみアトミックにリネームする（stage → 確認 → commit の二段階。置換は旧/新 SHA256 の並記 + 明示確認、公式ハッシュ不一致は二段階確認）。排他はプロセス全体で単一飛行。サイズ上限（仮値 100 MiB）は Content-Length の事前拒否 + ストリーミング累積打ち切りの両方で強制し、空き容量は「書き込み完了後も 1006 の警告閾値（1 GiB）を下回らない」判定で受け付ける。孤児ステージングは起動時 + 新規アップロード開始時に自動削除。**転送はブラウザの `fetch`（`wwwroot/js/forwarder-msi-upload.js`）による専用エンドポイント（`POST /admin/forwarder-kit/msi/stage`。antiforgery ヘッダー必須・`ForwarderMsiUploadPolicyName` 認可——ADR-0021 決定 1）で行い、SignalR circuit には載せない**（circuit のメッセージサイズ上限の緩和は禁止）
- **ACL 検出の二系統（決定 2・委任 7）**: ①乖離警告（1033）= opt-in 無効 + 書き込み ACE 残存（閉じ忘れ）、②開放継続の通知（1034。情報レベル・継続判定 24 時間・抑制窓 7 日——いずれも仮値）= opt-in 有効 + ACE 存在の継続。起動時の一回検査（Program）と周期監視（`ActiveNotificationMonitor`）の両方から発火する。**周期の判定は ACL の読み取りのみ**（`ForwarderMsiFolderAclInspector`——実書き込みプローブを毎分打つと SACL 監査を汚染するため）。生成画面は開放中であることを常時表示する。**アカウント切替時に手動付与した本フォルダ書き込み ACE の実挙動は AD lab（決定 5 lab ④。2026-07-25）で実測確定した——切替の除去レール（§5.2 の deferred CA `icacls … /remove:g *S-1-5-80-<旧アカウント SID>`）は旧アカウントへ手動付与した ACE も SID 単位で一掃する**（仮想 SA へ手動付与した `(M)` ACE が gMSA 切替後の forwarder から消失。残置 0）。したがって切替後に旧アカウントの書き込み残骸は残らず、**乖離検出（1033）は切替経路では空振り＝安全側**（委任 7 の期待値はこれで確定。ADR-0020 委任 7 参照）
- **監査（決定 4）**: 配置 = 2026・削除 = 2027・失敗/拒否 = 3014（§4.3。§4.4 の集約対象。ADR-0021 で `reason=operation-auth-required`——専用ポリシーの認可拒否——を 3014 の理由コードとして追加）。操作単位の実認証（ADR-0021 決定 1）により、実行された配置・削除の「誰が」欄（認証方式 + 利用者名）に**必ず値が入る**——PR #167 の却下理由（操作者特定の構造的不成立）の構造的解消（担保がリスナ全体の認証必須から専用ポリシーへ移っても、ポリシーを通過した操作は定義上すべて認証済み）。ACE 付与・撤去そのものは OS 側の操作であり Yagura は監査を偽装しない（SACL への誘導で対応）
- **公式ハッシュの来歴（委任 6）**: 照合基準（`ForwarderMsiConstraints.OfficialSha256ForVerifiedVersion` 系）の取得手順・確認日は同定数の doc コメントに記録済み（検証済み版の更新トリガ——各リリース準備時 + CVE 公表時——と同期して更新する。conventions.md の版運用）。照合対象は検証済み単一版のみであり、それ以外は常に「不一致」側のフロー（二段階確認）に乗る

### 5.2 gMSA 実行時の ACL 付与・検証（[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 4。gMSA opt-in 有効時）

§5・§5.1 の ACE 構成は、サービスの実行アカウントが既定の仮想サービスアカウント（`NT SERVICE\Yagura`）である前提で書かれている。**AD 環境向けの gMSA 実行（opt-in。指定方式は configuration.md §4.4）を有効化すると、実行アカウントがドメイン採番の gMSA（`DOMAIN\name$`）に変わり、ACL の付与機構と起動時検証の期待値がその実行アカウントに追随しなければならない**（ADR-0004 決定 5 の最小権限がネットワーク越し・ドメイン主体でも成立するため）。既定（仮想 SA）の体験・ACE 構成は一切変わらない。

- **ACL 付与機構は実行アカウントで分岐する（lab スパイクで確定。ADR-0015 改訂履歴 1 (iii)）**: §5・§5.1・configuration.md §4.3 のフォワーダフォルダはいずれも WiX の静的 SDDL 埋め込み（`PermissionEx` = `MsiLockPermissionsEx`）で ACE を設定する。この方式は**セキュリティ記述子をビルド時に確定する**ため、well-known SID（`SYSTEM`・`Administrators`）とビルド時に解決できる仮想サービスアカウントには使えるが、**ドメイン採番の gMSA SID には適用できない**。lab で実証した失敗形——gMSA でサービスを起動すると、プロセスは起動するが**データルートを開けずクラッシュする**（`Microsoft.Data.Sqlite.SqliteException ... SQLite Error 14: 'unable to open database file'`）。このときのデータルート ACL は**ビルド時に埋め込まれた仮想アカウントの SID がそのまま残り、名前解決もされず**、gMSA はどの ACE にも該当しないためアクセスできない:
  ...
  **後追い付与 ACE の実機 `icacls` 出力（AD lab 実測。2026-07-24。Windows Server 2025 `10.0.26100`・`yagura.test` DC・gMSA `YAGURA\gmsaYagura$`。`PermissionEx` の静的 SDDL 出力を §5 に記録済みなのと同じ扱いで本節に記録する）**:
  ...
  ```text
  C:\ProgramData\Yagura                YAGURA\gmsaYagura$:(OI)(CI)(M)
                                       NT AUTHORITY\SYSTEM:(OI)(CI)(F)
                                       BUILTIN\Administrators:(OI)(CI)(F)

  C:\ProgramData\Yagura\forwarder      YAGURA\gmsaYagura$:(OI)(CI)(R)
                                       NT AUTHORITY\SYSTEM:(OI)(CI)(F)
                                       BUILTIN\Administrators:(OI)(CI)(F)
  ```

  データルートは Modify `(OI)(CI)(M)`、フォワーダは読み取りのみ `(OI)(CI)(R)`。**静的 SDDL が直前に張った仮想 SA（`S-1-5-80-…`）の ACE は `/remove:g` により除去され、データルート配下の全再帰対象で残存 0**（切替経路でも同じ——SEC-14 (d) 参照）。既存の子ファイル（`yagura.db`・スプールセグメント等）も継承（`(I)`）で新アカウントの ACE を受ける。

- **起動時 ACL 検証の期待値は実効実行アカウントから導出する（ADR-0015 決定 4）**: §5 の起動時 ACL 検証（期待と異なる ACL を能動通知で警告する機構）は、**付与先を固定名（`NT SERVICE\Yagura`）でハードコードせず、現在の実効実行アカウントから期待 ACE を導出する**。固定名のままだと gMSA 環境では正しい ACL に対して常時誤警報を出し（`DOMAIN\name$` の Modify ACE を「想定外」と誤判定）、かつ gMSA 環境でのドリフト（本来あるべき gMSA ACE の消失）を見逃す——両方向の破綻を防ぐため、比較の基準を実行アカウントに追随させる。

- **アカウント切替時は旧アカウントの ACE を除去する（ADR-0015 決定 4）**: 実行アカウントを切り替える（再インストール／アップグレードでの `YAGURA_SERVICE_ACCOUNT` 変更。configuration.md §4.4）際、新アカウントへの後追い付与に加えて**旧アカウントの ACE を除去する**——権限の残骸（旧識別が保存データへアクセスできる状態）を残さない。**除去の責務は後追い付与側（`icacls`）が担う**——`PermissionEx` の静的 DACL は新規インストール時にしか再適用されないため、稼働継続をまたぐ切替経路（仮想 SA → gMSA・gMSA → 仮想 SA の両方向）での ACE 追加・除去はいずれも `icacls` 後追いが行う（新規インストールでは旧 ACE 自体が存在しないため除去は不要）。**除去に失敗した場合はその場でイベントログに残す**——起動時 ACL 検証（§5。実効実行アカウントから期待 ACE を導出）が旧アカウントの残置 ACE を「想定外の ACE」として検出し、能動通知（§5 の警告経路）で可視化する。operations 手順書への静的記載だけに委ねず、残置が起きた実機で運用者が気づける経路を持たせる（除去できないケースが lab〔ADR-0015 決定 7 (d)〕で判明した場合は、残置の事実と理由を §8 SEC-D8・本節にも明示する）。あわせて**切替後の初回起動で旧アカウント時代のスプール退避分が drain できる**こと（既存スプールファイルへの新アカウントの継承反映）を確認する——読めなければ退避済みログのロスに直結するため、決定 7 (d) の受け入れ条件に含める。

  **実装参照（Issue #263）**: 旧アカウントの値は remember property のレジストリ記録（`HKLM\SOFTWARE\Yagura\ServiceAccount`）から `YAGURA_SERVICE_ACCOUNT_PREVIOUS` プロパティへ読み込み、切替時（旧 ≠ 新・旧が gMSA）に deferred CA が `icacls /remove` で除去する（`Package.wxs`。`Return="ignore"` のベストエフォート——旧 gMSA が AD から削除済みで名前解決できないケースで切替そのものを失敗させない）。除去失敗時の運用者への可視化は、現時点では Host 側の限定チェックが担う——`ServiceAccountStartupInspector` がアカウント変化（2025）を検出した起動時に限り、データルートに旧アカウントの ACE が残っていないかをベストエフォートで照合し、残置を警告する（`[service-account-old-ace-remains]`。旧アカウントが名前解決できない場合は照合不能をその旨情報ログに残す）。§5 の起動時 ACL 検証（期待 ACE 全体の突合・能動通知）は未実装であり、実装時に本節の期待値導出（実効実行アカウント基準）を要件として引き継ぐ。切替経路の全域性（既存データ・スプールの継承反映を含む）の実機確認は SEC-14 (d)。

- **秘密鍵アクセス権の付与先も実行アカウントに追随する**: 証明書の秘密鍵読み取り権限の自動付与（§2.5・§6。監査 2009/2010）は、付与先を実行アカウントから導出する（gMSA 構成では `DOMAIN\name$` へ付与する）。付与機構自体（CNG 鍵ファイルへの ACE 追加。対応範囲・フォールバックは §2.5）は実行アカウントの種別に依存しない。
...
- **証明書ロード・秘密鍵アクセス権付与・期限監視の共有**: TLS 受信は `Yagura.Host.Administration.Https.CertificateProvider.Load` / `CertificatePrivateKeyAccessGranter.TryGrantReadAccess`（いずれも管理リスナのリモート HTTPS——§2.5——と共通の実装）をそのまま呼ぶ。設定キーは独立（`Ingestion:Tls:*`）——同一証明書の流用は両方のキーに指定することで実現し、暗黙の連動はしない
...
| SEC-9 | ~~AD グループの指定形式（名前 / SID）とネストグループ解決の有無~~ → **ADR-0010 Phase 4（2026-07-12）で確定・実装済み**。名（`DOMAIN\Group`）と SID の両形式を受理し、起動時に SID へ解決してキャッシュ（解決不能な指定は警告してスキップ＝認可を付与しない安全側）。ネストは `WindowsIdentity.Groups`（推移的グループ SID・OS 展開済み）で解決し追加 LDAP 不要。照合はトークンのグループ SID 集合と設定 SID 集合の交差。役割: `Admin:...:AdminGroups`→管理（544 に加算）、`Viewer:...:ViewerGroups`→閲覧、`Viewer:...:AdminGroups`→管理（§3）。**lab 実機確認済み（2026-07-18。yagura.test ドメイン / Yagura 0.4.0）**——①**名前形式 + ネストグループ**: `Viewer:...:ViewerGroups = YAGURA\YaguraViewers` に対し、その入れ子グループ `YaguraNestedViewers` にのみ所属する利用者が `200` + 「役割: 閲覧」を得ることを確認（推移的解決が実際に働く） ②**SID 形式**: `Viewer:...:AdminGroups` に SID 直指定したグループへ利用者を追加すると「役割: 管理（閲覧を含む）」へ昇格することを確認 ③**非所属者の拒否**: いずれのグループにも非所属かつ `BUILTIN\Administrators` 非該当の利用者は `403` で拒否され、監査 3009 が記録されることを確認（`{"Kind":"ViewerAuthorizationDenied","EventId":3009,"AttemptedPath":"/login/windows","ReachedListenerPort":8514,"Detail":"authenticated-but-not-in-viewer-or-admin-group","AuthenticatedPrincipal":"YAGURA\\yagura-none1"}`） ④**解決不能指定**: 存在しないグループ名を混在させても他の指定は正しく機能し、認可も付与されない（安全側で正しい）——警告 `[sec9-group-unresolved]` の到達は SEC-9-a を参照（発見時は届かない限界があったが解消済み） | §3 |
| SEC-9-a（新規。上記の派生） | ✅ **解消（Issue #346 / PR #354。2026-07-19）**——発見時は解決不能なグループ指定の警告が運用者に届かなかった（lab 実機確認 2026-07-18）。`WindowsSecurityGroupResolver.ResolveToSids` は `[sec9-group-unresolved]` を `LogWarning` で出力するが、その出力先は `Program.cs` の **bootstrap ロガー**（`bootstrapLoggerFactory`。ホストの EventLog プロバイダ構築前）であり、Windows イベントログへ到達しない。実測: 存在しない `YAGURA\NoSuchGroup12345` を設定して起動しても、イベントログ全体（Yagura プロバイダ 400 件）に当該警告は **0 件**。他の起動時警告（`Yagura.Host.Administration.Https` / `.ViewerAuth` 等）は到達しているため、ロガーの差に起因する。ファイルログの出力先も存在しない。**帰結**: グループ名のタイプミスは「そのグループの所属者が黙って認可されない」形で現れ、運用者は原因に辿り着く手がかりを得られない（fail-closed ではあるが不可視）。**解消の実装**: `WindowsGroupAuthorizationOptions` の構築を factory 登録へ変え、ロガーを DI の `ILoggerFactory`（EventLog プロバイダ構築後）から取得し、`app.Build()` 直後に一度解決して警告の発火時点を起動時に固定した。CI では検出できない（既存 E2E は stdout 監視で、修正前も bootstrap ロガーはコンソールへは出力していたため差が出ない）ため、回帰検証は lab 手順書（`installer/lab/startup-warning-regression-procedure.md`。#356）で行う | —（解消済み。回帰検証は lab 手順書） | §3 |
...
| SEC-13 | 管理リスナのリモート HTTPS 証明書の秘密鍵権限付与（ADR-0010 Phase 2 決定 4）の実機確認 | **確定（①②③すべて実施。lab 実機検証 2026-07-18。Windows Server 2025 / Yagura 0.4.0 / インストーラ登録の `NT SERVICE\Yagura`）**。詳細は §2.5「秘密鍵権限付与の実機検証結果」。要点: **自動付与は失敗する**（サービスアカウントは鍵ファイルの ACL を書き換える権限を持たないため）。**権限が全く無い状態でも起動は継続する**（発見時は `CngKey.Open` の未処理例外で起動しない不具合だったが、Issue #345 で 2026-07-19 修正済み——例外を捕捉し手動付与誘導の警告へ変換。回帰検証は lab 手順書 #356。§2.5 ③ 参照）。**読み取り権限を別途付与すれば TLS は成立する**（③では AD CS の信頼チェーン検証込みで成立）。**監査 2009 は記録されない**（付与成功時のみ発火するため）。**③ AD CS 発行証明書（エンタープライズ ルート CA + `WebServer` テンプレート + CNG KSP）でも①②と完全に同一の結果**——本項の結論は**証明書の発行元に依存せず**、決定要因は鍵ファイルの既定 ACL にサービスアカウントの ACE が無いことである | §2.5 |
| SEC-14 | gMSA 実行（[ADR-0015](../adr/0015-gmsa-service-account.md)。opt-in）の AD lab 実機 E2E 検証（決定 7）。**実装最初期の lab スパイク（決定 2——`ServiceInstall/@Account` のプロパティ間接参照成立・ACL 後追い付与必須）は 2026-07-18 に実施済み**（ADR-0015 改訂履歴 1。§5.2 に機構を反映済み。スパイクが「必須」としていた `SeServiceLogonRight` 明示付与は **2026-07-24 の受け入れ検証で検証済みビルド 26100 では不要と実測確定し「推奨」へ訂正**——ADR-0015 改訂履歴 2・Issue #422）。**残るのは実装完了後の E2E 受け入れ**（決定 7 (a)〜(f)）: (a) 新規インストール（プロパティ指定）→ 受信 → 閲覧 → リモート SQL Server への統合認証接続、(b) 強制パスワードローテーション（`Reset-ADServiceAccountPassword`）を跨ぐ受信・DB 書き込みの継続、(c) DC 停止状態でのサービス再起動（1069 の実挙動・SCM 回復設定の実効）、(d) 既存データを持つアップグレードでのアカウント変更（ACL 付替の全域性・旧 ACE 除去・スプール退避分の drain・失敗時着地。§5.2）、(e) 監査証跡（2024/2025 の転記・変化検出、秘密鍵付与先、起動時 ACL 検証が誤警報しないこと）、(f) gMSA への HTTP SPN 登録手順の確定 + `SeServiceLogonRight` 要否のビルド差の記録（26100 = 不要を実測確定〔2026-07-24。改訂履歴 2・Issue #422〕。「1920 ロールバックの観測」は現行ビルドで成立不能のため受け入れ項目から除外——gMSA 固有の実失敗モードは (c) の 1069 が受け皿）。**CI（AD なし）で継続検知する範囲**（決定 7）: アカウント形式検証ロジックの単体テスト・MSI テーブル照合（プロパティ既定値・間接参照の展開）・仮想 SA 経路の非退行（installer-e2e）。lab でしか検証できない項目は installer/README の検証マトリクスに記載する | 実装 PR の受け入れ条件（AD lab。yagura.test DC）。lab スパイク（決定 2）は実施済み。**(a)/(c) は 2026-07-24 に実測済み**（[ADR-0015](../adr/0015-gmsa-service-account.md) 改訂履歴 3——統合認証接続・書き込みの成功、接続失敗 3 態様の SqlException 実測〔1031 の分類根拠〕、DC 停止時の挙動〔キャッシュ済みチケット有効中は接続成功・停止中の再起動は 1069 で失敗し DC 復旧後の自動再起動で回復〕）。**(b)(d)(e) は 2026-07-24 に実測済み**（[ADR-0015](../adr/0015-gmsa-service-account.md) 改訂履歴 4）——(b) ローテーション跨ぎで受信・SQL 書き込み無瞬断・1030/1031 不発、(d) 両方向の切替で ACL 付替の全域性〔既存子ファイルの継承反映〕・旧 ACE の `/remove:g` 除去・スプール drain・2025 発火・失敗時の元アカウント/元 ACL へのクリーン着地、(e) 2024（初回 1 回・再起動で重複なし）/2025（インストーラ切替・`sc config` 切替の双方）/`[service-account-old-ace-remains]` 警告。**ただし (d) のサイレント切替は素の remember-property では AppSearch がコマンドライン値を上書きして無言 no-op になる欠陥（#426）を検出し、CLI 上書き対応（PR #427）で修正のうえ実測した**。(f) は HTTP SPN 登録手順・重複事前確認を確定し、**別マシンからの Kerberos SSO 成立を実測**（gMSA の SPN で `klist` に `HTTP/<FQDN>` チケット・同一マシンは NTLM 後退。§2.5）。(e) の 2009/2010 は付与先が実効アカウント gMSA に追随することを確認（2009 の実発火は SEC-13 と同じ手動付与を要する制約——実行アカウント種別に非依存）。残りは (f) の認可済み管理者 Kerberos による監査 2008 `scheme=windows` の E2E（ADR-0013 の Cookie/circuit 自動化の残件と重複）のみ | §5.2・§4.3（2024/2025・1031）・[ADR-0015](../adr/0015-gmsa-service-account.md) |
...
| SEC-D7 | 管理リスナのリモートバインド（ADR-0010 Phase 2）: ①Windows 統合認証は**ホスト名アクセスではマシンアカウントの `HOST` SPN 経由の Kerberos SSO が成立する見込み**（#228 でマシンアカウント Kerberos の動作を確認。`HTTP`⊂`HOST` の機序と E2E の確定は ADR-0010 委任事項 3 / Phase 3。§2.5）だが、**IP アドレスアクセス・非ドメイン機では NTLM 経路になり、CNAME・別名・ロードバランサ経由の名でアクセスする構成は当該名への SPN 登録を別途要する**こと（NTLM の既知の弱点——リレー攻撃・pass-the-hash・相互認証の欠如——を明示し、ホスト名アクセスと Kerberos-only モードで Kerberos を強制できること・別名構成の SPN 登録手順を案内する）、②有効化に要するファイアウォール規則の手動作成手順（規則名の名前空間・ポート——既定 8516——を含む。インストーラは自動作成しない）、③証明書取得・証明書ストアへの取り込み手順（configuration.md §6 CF-D2 と同型。ひとり情シス向けの最短経路を含む）、④暗号スイートの個別制約は製品側で提供しないこと（Windows では .NET の `CipherSuitesPolicy` が使えないため——OS レベルの TLS ポリシー・グループポリシーでの統制が必要な場合の案内先）、⑤証明書の秘密鍵アクセス権の自動付与が対応する秘密鍵の種別（CNG ソフトウェアキーストレージプロバイダーのみ）と、対応外（スマートカード・HSM・TPM 保護鍵）の場合の手動付与手順 | §2.5・[ADR-0010](../adr/0010-admin-ui-authentication.md) Phase 2 |
| SEC-D8 | **AD 環境向け gMSA 実行（[ADR-0015](../adr/0015-gmsa-service-account.md)。opt-in）の運用ガイド**: ①gMSA を推奨する条件（リモート SQL Server + DB 側の最小権限付与を要する AD 環境。SQLite のまま・SQL Server 同居・マシンアカウント許可で足りる環境は仮想 SA のままでよい——判断フロー。決定 5）、②AD 側の前提条件は AD 管理者の責務であること（gMSA の作成・取得許可 `PrincipalsAllowedToRetrieveManagedPassword` のホスト単位への最小化・KDS ルートキーの初回作成と既定 10 時間待ち・`Test-ADServiceAccount` での確認。決定 3）、③`SeServiceLogonRight` の明示付与が必須であること（gMSA は既定で権利を持たず、付与しないとサービスが一度も起動しない——lab 実証。configuration.md §4.4）、④起動失敗（1069 系）の 3 段チェックリスト（`Test-ADServiceAccount` → 取得許可 → グループ経由時のホスト再起動。決定 3）、⑤gMSA 構成ではサービス起動が DC 到達性に依存すること（停電復旧時の起動順・SCM 回復設定の見直し。決定 3・帰結 2）、⑥稼働中の切替手順（サービス停止 → `sc config` → ACL 付替 → 起動。DPAPI machine スコープは同一マシン内の切替で影響しないこと・保存データは保持されること。決定 6）、⑦定期確認（`Test-ADServiceAccount`）の推奨（AD 側の取得許可の事後変化は次回起動の 1069 まで顕在化しない時限性。帰結 3）、⑧**gMSA 採用時の HTTP SPN 登録手順と Kerberos 退行の注意**（リモート管理 HTTPS をホスト名 + Kerberos SSO で使う構成が gMSA へ切り替わると、gMSA は `HOST/hostname` の自動登録に与らないため、`HTTP/hostname` を gMSA へ明示登録しない限り Kerberos SSO が NTLM へ後退する。決定 7 (f)。§2.5）、⑨**DB 最小権限は v0.1 では段階的**（v0.1 の提示 SQL は導入時権限として `db_owner` を付与するため、gMSA を使っても DB 側の最小権限は実行時最小権限への分離の実装まで実効化しない。database.md §6.1・§5.2）、⑩**サービス死活の外形監視の推奨**（gMSA 構成はサービス起動が DC 到達性に依存し〔帰結 2〕、DC 断による 1069 起動失敗時は閲覧 UI も上がらないため Yagura 自身では構造的に検知できない——CF-D8 と同じく、**Yagura 以外の経路でのサービス状態・外形監視を推奨**し、監視すべきイベント〔1069 系のシステムログ・サービス停止状態〕を案内する。定期確認 `Test-ADServiceAccount`〔⑦〕と併せる） | §5.2・§2.5・[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 3・5・6・7 (f)・8・帰結 2 |
