...
- **ユーザー列挙耐性・ロックアウト**（委任事項 4）: 存在しないユーザー名でも実在アカウントの誤パスワードと同一の応答・同等のハッシュ検証コスト（ダミーハッシュに対する PBKDF2 検証）を払う。ロックアウト閾値・期間は仮値（[SEC-12](#7-確定待ち一覧)）。失敗試行カウンタの増分は単文の原子的 UPDATE（`FailedAttemptCount = FailedAttemptCount + 1` + 条件付きロックアウト設定を SQLite は `RETURNING`、SQL Server は `OUTPUT` で単一文に畳む）で行い、並行するログイン失敗（分散送信元からの同時試行）での lost update——閾値到達の遅延——を構造的に防ぐ。CSRF 対策はログイン POST エンドポイントで `IAntiforgery.ValidateRequestAsync` を明示検証する
  - **（設計変更が決定済み。実装 PR で本文差し替え）** 上記の「一時ロックアウト（一定時間の完全拒否）」および「原子的 UPDATE による DB カウンタ増分」は、[ADR-0010 Amendment (2026-07-11)](../adr/0010-admin-ui-authentication.md) により **アカウント単位の指数バックオフ（完全拒否をやめ遅延に置き換え。キーは (アカウント × loopback/remote)）+ 送信元 IP レート制限 + グローバルなトークンバケット**（状態はインメモリ・原子更新）へ改める決定がなされた。**現時点の実装はハードロックアウトのまま**であり、本記述と§4.3 の ID 3005・§7 SEC-12 の当該記述は実装 PR で当該機構の記述へ差し替える（設計書の「常に現在形」を保つため、未実装の挙動を先に現在形で書かない）
...
| 3005 | 3000 番台（拒否・セキュリティ事象） | 警告 | 管理者アカウントをロックアウト | アプリ独自認証アカウントのロックアウト発生（ADR-0010 決定 6。連続失敗試行が閾値——SEC-12——に達した）。**「解除」側の ID は Phase 1 では採番しない**——ロックアウト解除は時間経過による自動失効のみで「解除」という個別事象が発生する時点が存在しないため（明示的な解除操作も未実装。管理者による解除操作を実装する際に採番する——§2.4 の申し送り）。**（凍結予定）本 ID の意味・レベルは変更しないが、[ADR-0010 Amendment (2026-07-11)](../adr/0010-admin-ui-authentication.md) のバックオフ方式採用以降は発火しなくなる（旧ハードロックアウトの凍結値。発火 0 は意味の転用ではないため additive-only 規約に反しない）。バックオフ上限到達・IP レート制限拒否は実装 PR で新規 ID（3006 以降）として採番する** |
...
- **TLS ハンドシェイク失敗カウンタ**: `yagura.ingestion.tcp.tls_handshake_failure`（architecture.md §4.1.1。送信元アドレスをタグに持つ——本カウンタ体系で唯一のタグ付き計器。理由は同表参照）。`TlsSyslogListener` がハンドシェイク失敗（`AuthenticationException`/`IOException`/`OperationCanceledException`、およびハンドシェイク猶予超過）のたびに計上する
...
| SEC-12 | アプリ独自認証のブルートフォース保護のパラメータ。**ADR-0010 Phase 1 で仮値（ハードロックアウト 閾値 5 回 / 期間 15 分。`AdminAuthenticationDefaults`）のまま実装済み**。**[ADR-0010 Amendment (2026-07-11)](../adr/0010-admin-ui-authentication.md) で機構をバックオフ + IP レート制限へ改める決定済み**——本項の対象を「バックオフの k・base・cap、IP レート制限の窓・上限、グローバルバケットの秒間上限、能動通知の継続閾値、パスワード最小長」へ読み替える（実装 PR で確定・反映） | 仮値で実装し、実利用（誤入力の反復頻度等）を踏まえ確定する | §2.4 |
