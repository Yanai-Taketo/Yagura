...
| SEC-3 | ACL の具体 ACE 構成（icacls 出力の形で記録）。**監査記録領域の追記専用 ACE の成立可否・ローテーション権限の分離を含む**。削除・ローテーション機構（Issue #261）は分離構成を前提に設計済み（日次ファイル分割 = 書き換え不要・ACL 拒否時は警告して運用手順へ委譲——§4.2）。検証手順書: `installer/lab/sec-3-audit-acl-procedure.md` | 実機検証（仮想サービスアカウントの指定形式・継承フラグを含む。lab = サービスがインストールされた環境で実施） | §4.2・§5 |
