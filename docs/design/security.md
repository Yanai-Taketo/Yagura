...
- **①IP レート制限**: 送信元 IP 単位の固定窓（仮値 60 秒 / 10 回。[SEC-12](#7-確定待ち一覧)）。超過時は待たせず即座に 429 + 有限 `Retry-After`（仮値上限 30 秒）で拒否する
...
- **応答種別・UI 文言の統一**（決定 3・6）: 実在アカウントのバックオフ待機中・非実在ユーザー名・IP レート制限/グローバルバケット拒否のいずれも、利用者応答（結果種別）・ログイン画面の待機表示を区別しない——単一の一般文言 + カウントダウン（「しばらくお待ちください。あと X 秒で再試行できます。」）を表示し、原因（連続失敗のため等）は明かさない。失敗理由・拒否層の別は監査記録にのみ残す
