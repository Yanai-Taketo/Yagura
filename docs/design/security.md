...
- **Kerberos SPN の成立と残る NTLM 経路（#228 実測で更新。[ADR-0015](../adr/0015-gmsa-service-account.md) 決定 8）**: ADR-0010 起案時は「Kerberos SPN の仮想サービスアカウントでの成立可否は Phase 3 まで未検証」としていたが、**#228 の AD lab 実機検証（2026-07-11）で、仮想サービスアカウント構成のまま正規の SPNEGO-Kerberos・Kerberos AP-REQ が誤遮断されず疎通することを確認した**（§2.4 の Kerberos-only モード検証。Kerberos の発券が成立していた＝**マシンアカウントの `HOST` SPN 経由の Kerberos が動作している**ことを示す。Windows は `HOST/hostname` を機のコンピューターアカウントへ自動登録し、仮想サービスアカウントはネットワーク上その識別で認証するため、`HTTP` を含む `HOST` 傘下のサービスに対しホスト名アクセス時の Kerberos SSO が追加登録なしに成立する）。したがって:
  - **ホスト名でのリモート管理アクセスは Kerberos SSO が成立する**（SPN 未登録を前提にした NTLM 常用の想定は、ホスト名経路については成り立たない）
  ...
  - **SPN 起因での gMSA 要求は消えた**: ADR-0010 委任事項 3 の「仮想サービスアカウントへの SPN 登録が成立しない場合、gMSA との接続で再評価する」は、成立が確認されたため**決着**した（gMSA が SPN のために必要になることはない）。gMSA は別の理由——リモート SQL Server への DB 側最小権限付与・保存/監査の主体をドメイン管理体系に載せること——で opt-in 提供する（ADR-0015。§5.2・database.md §6.1）
...
| 2025 | 2000 番台（管理操作の監査） | 情報 | — | サービス実行アカウントが前回起動時から変化 | 起動時に実効実行アカウントが前回起動時の記録（`last-service-account.json`）と異なる場合に 1 件記録する（ADR-0015 決定 8。§4.1）。製品外の `sc config` による切替（決定 6 の稼働中切替手順）も含めて次回起動で必ず証跡化する。記録内容は旧・新の実行アカウント名——「いつからこの識別で動いているか」に監査で答える。初回起動・記録欠損時は発火しない（転記 2024 のみ）。2019（設定ファイルの変化検出）と同じ「変化検出」レールだが、対象が実行アカウントの識別である点が異なる |
