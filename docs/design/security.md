...
| SEC-14 | gMSA 実行（[ADR-0015](../adr/0015-gmsa-service-account.md)。opt-in）の AD lab 実機 E2E 検証（決定 7）。**実装最初期の lab スパイク（決定 2——`ServiceInstall/@Account` のプロパティ間接参照成立・`SeServiceLogonRight` 明示付与必須・ACL 後追い付与必須）は 2026-07-18 に実施済み**（ADR-0015 改訂履歴 1。§5.2 に機構を反映済み）。**残るのは実装完了後の E2E 受け入れ**（決定 7 (a)〜(f)）: (a) 新規インストール（プロパティ指定）→ 受信 → 閲覧 → リモート SQL Server への統合認証接続、(b) 強制パスワードローテーション（`Reset-ADServiceAccountPassword`）を跨ぐ受信・DB 書き込みの継続、(c) DC 停止状態でのサービス再起動（1069 の実挙動・SCM 回復設定の実効）、(d) 既存データを持つアップグレードでのアカウント変更（ACL 付替の全域性・旧 ACE 除去・スプール退避分の drain・失敗時着地。§5.2）、(e) 監査証跡（2024/2025 の転記・変化検出、秘密鍵付与先、起動時 ACL 検証が誤警報しないこと）、(f) `SeServiceLogonRight` の成立主体・SPN 付け替え手順の確定。**CI（AD なし）で継続検知する範囲**（決定 7）: アカウント形式検証ロジックの単体テスト・MSI テーブル照合（プロパティ既定値・間接参照の展開）・仮想 SA 経路の非退行（installer-e2e）。lab でしか検証できない項目は installer/README の検証マトリクスに記載する | 実装 PR の受け入れ条件（AD lab。yagura.test DC）。lab スパイク（決定 2）は実施済み | §5.2・§4.3（2024/2025）・[ADR-0015](../adr/0015-gmsa-service-account.md) |
