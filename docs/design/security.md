...
| 1009 | 1000 番台（運用警告） | エラー | スプールの定期自己検証に失敗 | スプールの定期自己検証（architecture.md §3.2.5。Issue #152）が失敗した——合成レコードの投入自体に失敗した（書込失敗・上限到達）、または投入した合成レコードが期待時間（仮値 10 分。architecture.md §9 M-16）以内に drain へ合流判定されなかった。タイムアウトの場合、原因は経路の障害と未消化バックログの滞留（持続的な速度不足。architecture.md §3.2.2 が正常状態と明記する運用状態——この場合は 1004 が並行発火している可能性が高い）の両方があり得るため、本文は断定せず両論併記とし、現在のスプール使用率を切り分けの手がかりとして含む（PR #200 レビュー対応。バックログ起因の判定での区別はフォローアップ課題）。障害時専用経路（スプール退避 → drain）の平常時検証が完了していない状態のためレベルはエラー（本節のレベル割当方針「機能停止を伴う事象」）。`ActiveNotificationMonitor` による周期監視（Issue #152） |
...
以後、他の 3000 番台事象（認証失敗・失効猶予の記録等。§4.1 の対象一覧）を実装する際は 3003 以降を、2000 番台の管理操作は 2006 以降を、1000 番台の運用警告（HTTPS 証明書の期限接近、circuit 上限の到達等。いずれも本 PR のスコープ外。自己検証の失敗は Issue #152 で 1009 として実装済み）を実装する際は 1010 以降を additive-only で追加し、本表を更新する。
...
MSI オプトイン同梱のため、管理者が Fluent Bit の MSI を手動配置するフォルダ（データルート配下 `forwarder`。`%ProgramData%\Yagura\forwarder\`）は、§5 の ACL 方針に準拠し**既定で `Administrators` のみ書き込み可**とする——ここに書ける者は「全端末に配る MSI を差し替えられる者」であり、供給網上防御価値が最も高い書き込み対象になるため（ADR-0008 設計条件 9）。

- **本 PR（フォワーダキット生成の実装）はフォルダの作成・ACL 設定を行わない**: Web 層は配置フォルダを読み取るのみであり、フォルダが存在しない場合は「未検出」として扱う（作成しない）。フォルダ作成・ACL 適用はインストーラ（WiX）の領分であり、既存データルートの ACL 継承（§5）を前提とする
- **WiX 側での明示的なフォルダ作成・ACL 設定は本 PR の範囲外**であり、別 PR の検討事項として残る（申し送り。データルート配下である以上 §5 の ACL が継承されることを既定の前提とするが、`forwarder` サブフォルダを明示的に作成し ACE を固定するかどうかは未検証）
