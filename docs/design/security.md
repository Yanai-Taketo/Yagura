...
- **有効化条件（決定 1）**: `Admin:ForwarderKit:MsiUpload:Enabled = true` は、管理 UI 認証（§2.4）のいずれかが有効 **かつ** `Admin:Authentication:RequireForLoopback = true`（= 管理リスナに無認証の到達経路が存在しない）の構成でのみ受理される。満たさない組み合わせは起動時 fail-closed（イベント ID 1032。`YaguraConfigurationLoader.Load`——1011/1012 と同型）。**lab 観測の留意（決定 5 lab P-1。2026-07-25。[Issue #433](https://github.com/Yanai-Taketo/Yagura/issues/433)）**: この fail-closed は Host 構築前の bootstrap ロガー（`Program.cs` の `AddConsole()` 専用）で `LogCritical(1032)` したのち `ConfigurationValidationException` を re-throw する。**Windows サービスとして動かすと 1032 は Windows イベントログに現れず**、管理者がイベントログで見るのは `.NET Runtime` 1026 + Application Error 1000（例外本文に欠落条件・誘導文言は入る）になる——これは 1032 固有でなく Host 構築前の全 config 検証 fail-closed（1011/1012/1024/1032）に共通。サービス構成での観測性の是正は #433 で追跡。UI 側（生成画面）でも同じ条件を先に判定し、不成立時は機能を表出させず「どの条件が欠けているか + 手動配置ガイドへの導線 + `RequireForLoopback` 有効化の代償（復旧 = 手編集 + 再起動 = 受信断）とブレークグラス強推奨」を案内する。**アップロード関連エンドポイントは条件成立時のみ登録する**（構造的非存在——`MapYaguraAdmin` の条件分岐。無効構成では 404）
...
- **監査（決定 4）**: 配置 = 2026・削除 = 2027・失敗/拒否 = 3014（§4.3。§4.4 の集約対象）。前提条件により全経路が認証済みのため、「誰が」欄（認証方式 + 利用者名）に**必ず値が入る**——PR #167 の却下理由（操作者特定の構造的不成立）の構造的解消。ACE 付与・撤去そのものは OS 側の操作であり Yagura は監査を偽装しない（SACL への誘導で対応）
