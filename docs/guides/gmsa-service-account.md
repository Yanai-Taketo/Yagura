...
- SEC-14 (c) の lab 実測（2026-07-24）では、**DC 停止中のサービス再起動は 1069 で失敗し、DC 復旧後の SCM 自動再起動（2 回目）で回復する**こと、また**キャッシュ済み Kerberos チケットが有効な間は稼働中の DB 接続が DC 停止でも成功する**ことを確認済みです（[ADR-0015](../adr/0015-gmsa-service-account.md) 改訂履歴 3）。回復設定の推奨値の確定は引き続き残項目です
