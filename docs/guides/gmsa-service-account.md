...
> **検証状態の注記**: SCM 回復設定の推奨値（§7）と HTTP SPN 登録手順（§8）は AD lab の受け入れ検証（security.md SEC-14）で確定済みです（2026-07-24。[ADR-0015](../adr/0015-gmsa-service-account.md) 改訂履歴 3・4）。SPN の NTLM 後退 → Kerberos SSO 回復の end-to-end 実測のみ残項目です（§8）。
