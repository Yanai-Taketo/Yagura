...
### 2.4 「サービスとしてログオン」権利（SeServiceLogonRight）の付与 — 推奨（強制は環境依存）

**gMSA へ `SeServiceLogonRight`（サービスとしてログオン）をローカルセキュリティポリシーまたは GPO で事前に付与しておくことを推奨します。** MSI のサービス登録も SCM もこの権利を自動付与しません。

**この権利の強制は Windows のバージョン・構成に依存します**:

- **強制されない実測（2026-07-24 AD lab。Windows Server 2025 / Windows 10 `10.0.26100`）**: gMSA に本権利を付与しなくても MSI インストールは成功し、サービスは gMSA として起動・受信した。`LsaEnumerateAccountRights` で gMSA の権利がゼロであること、権利を明示付与→**明示除去しても**再起動でログオンが成立すること（logon 失敗イベントなし）を確認済み。当該ビルドの SCM は gMSA の service logon に本権利を強制していない。
- **一方で、強制される（未付与だと起動が `1920`／`1069` で失敗し、インストールが `1603` でロールバックし得る）環境もある**（ADR-0015 改訂履歴 1 のスパイク実測。その 1920 の真因は再検討課題——[改訂履歴](../adr/0015-gmsa-service-account.md)参照）。

**したがって: 事前付与は「無害な防御的措置」として推奨する。起動が `1920`／`1069` で失敗した場合は、本権利の付与を最初の対処とする**（あわせて gMSA の取得許可〔§2.2〕・DC 到達性〔§7〕も切り分ける）。
...
このほか、インストール直後の起動失敗（エラー 1920）は、環境によっては `SeServiceLogonRight` の未付与が原因になり得ます（[§2.4](#24-サービスとしてログオン権利seserviceLogonrightの付与--推奨強制は環境依存)。付与は無害なので最初の対処として試す。ただし Windows Server 2025 / 26100 では本権利は強制されず、この場合の 1920 は gMSA パスワード取得失敗〔取得許可・§7 の DC 到達性〕を先に疑う）。
