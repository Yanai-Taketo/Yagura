...
2. **`Admin:Authentication:RequireForLoopback = true`**(サーバ上のブラウザから開く場合にも
   ログインを求める設定)。**この設定には代償がある**——認証手段がすべて使えなくなった場合の
   復旧は「設定ファイルの手編集 + サービス再起動(= その間の syslog 受信断)」になる。
   有効化する前に、Windows 統合認証とは別に**アプリ独自の管理者アカウントを最低 1 つ**
   作成しておくこと(非常口)を強く推奨する。また、認証を緊急無効化する手編集を行う場合は、
   **`Admin:ForwarderKit:MsiUpload:Enabled` も同時に `false` にする**こと(残したままだと
   fail-closed——イベント ID 1032——で起動しない。エラーメッセージにも同じ誘導が出る)
3. **`Admin:ForwarderKit:MsiUpload:Enabled = true`**(機能そのものの opt-in)
4. **書き込み経路の開放**(次節)

条件 1〜3 が揃わない構成では、生成画面のアップロード欄に「無効であること + どの条件が
欠けているか」が表示される(機能は現れない)。前提条件を課す理由は、管理リスナに認証なしで
到達できる経路が残っている間は「誰が MSI を差し替えたか」を記録できないためである
(全端末に配布されるファイルの書き込み口は、操作者を特定できる構成でのみ開く)。
