...
`install.ps1 -Mode` で選択できる(特性の違いは後述「UDP と TCP のどちらを使うか」参照)。
**TLS 暗号化転送(syslog over TLS。RFC 5425)は本キットでは提供しない**——Yagura 側は
RFC 5425 準拠の TLS 受信に対応するが、Fluent Bit の `out_syslog` が octet-counting
フレーミング非対応のため Yagura の TLS 受信の送信元にできない(後述「TLS 暗号化転送について」参照)。
...
主に手動・少数端末導入の場面に限られる。配置の経路は 2 つある: **(a) サーバの
ファイルシステムへの手動配置**(次節。どの構成でも使える)と、**(b) 管理画面からの
アップロード**(認証有効時限定の opt-in。[ADR-0020](../adr/0020-forwarder-msi-upload.md)。
後述「管理画面から MSI をアップロードする」)。

## 管理 UI の生成キットに MSI を同梱する(手動配置手順)

管理画面(`/admin/forwarder-kit`)の生成 ZIP に Fluent Bit の MSI を同梱したい場合、
サーバへ管理者権限で MSI ファイルを手動配置する(管理画面からのアップロード——後述の
経路 (b)——は認証構成等の前提条件を要するため、どの構成でも使える基本経路はこちら)。
配置は RDP・コンソールログオン・リモート PowerShell(管理者セッション)の
...
## 管理画面から MSI をアップロードする(配置経路 (b)。認証有効時限定の opt-in)

手動配置(前節)に加え、**管理画面から MSI をアップロードして配置する経路**を opt-in で
使える([ADR-0020](../adr/0020-forwarder-msi-upload.md))。反復する版更新(検証済み版の
更新は各リリース準備時と Fluent Bit の重大 CVE 公表時に発生する)を、認証・監査の効いた
管理画面の操作で完結させるための機能である。

### 前提条件(すべて必要)

1. **管理 UI 認証の有効化**(Windows 統合認証またはアプリ独自 ID/パスワード。
   [ADR-0010](../adr/0010-admin-ui-authentication.md))
2. **アップロード機能の有効化**(`Admin:ForwarderKit:MsiUpload:Enabled`)。
   **生成画面(`/admin/forwarder-kit`)の「アップロード機能を有効にする」から切り替えられる**
   (設定ファイルの手編集は不要)。切替にはサインインが必要で、**反映にはサービスの再起動
   (= その間の受信断)が 1 回必要**。アプリ独自認証のアカウントが既にある場合は、その
   ユーザー名・作成時刻・最終変更時刻が表示され、「自分(または信頼できる管理者)が作成した
   ものである」ことの確認を求められる——有効化前は、このサーバ上のプロセスがサインインなしで
   アカウントを作成できるためである(作成・変更の記録は監査に残る)
3. **書き込み経路の開放**(次節)

`Admin:Authentication:RequireForLoopback` の有効化は**必要ない**
([ADR-0021](../adr/0021-msi-upload-operation-auth.md) で前提条件から撤廃)。サーバ上の
ブラウザからの閲覧・通常の管理操作は従来どおりログインなしで使える。
**そのかわり、アップロード・削除の操作そのものにサインインが必要になる**——画面の
アップロード欄が「サインインが必要」の案内を出すので、そこからサインインして操作する
(Windows 統合認証を有効化していれば追加の入力なしで通る構成が多い)。全端末に配布される
ファイルの書き込み口につながる操作だけは、「誰が行ったか」を記録できるようサインインした
管理者に限定している。

条件 1〜2 が揃わない構成では、生成画面のアップロード欄に「無効であること + どの条件が
欠けているか」が表示される(機能は現れない)。なお、認証を緊急無効化する手編集を行う場合は、
**`Admin:ForwarderKit:MsiUpload:Enabled` も同時に `false` にする**こと(残したままだと
fail-closed——イベント ID 1032——で起動しない。エラーメッセージにも同じ誘導が出る)。

### 書き込み経路の開放と撤去(OS 管理者の明示操作)

Yagura は自分では配置フォルダの権限を変更しない。アップロード(および画面からの削除)が
実際に成立するのは、**OS の管理者が次のコマンドでサービス実行アカウントに配置フォルダ限定の
書き込み権限(ACE)を付与している間だけ**である:

```powershell
# 付与(既定の仮想サービスアカウントの場合。gMSA 構成では "DOMAIN\name$" に読み替える)
icacls "$env:ProgramData\Yagura\forwarder" /grant "NT SERVICE\Yagura:(OI)(CI)(M)"

# 確認(付与後は NT SERVICE\Yagura:(OI)(CI)(M) が表示される)
icacls "$env:ProgramData\Yagura\forwarder"
...
**開くなら見張ることを推奨する**: 付与とセットで、フォルダの書き込みを OS 監査(SACL)に
記録する設定を推奨する(プロセス侵害時、Yagura 自身の監査記録は信頼できないため、
独立した証跡は OS 監査と配布先に残る `GENERATED.txt` だけになる):

```powershell
# 書き込み系操作の監査(SACL)を Everyone に対して有効化する例
$acl = Get-Acl "$env:ProgramData\Yagura\forwarder" -Audit
$rule = New-Object System.Security.AccessControl.FileSystemAuditRule(
    "Everyone", "Write,Delete", "ContainerInherit,ObjectInherit", "None", "Success")
$acl.AddAuditRule($rule)
Set-Acl "$env:ProgramData\Yagura\forwarder" $acl
```

### 運用モードの選び方(どちらでもよいが、違いを理解して選ぶ)

| | 使うときだけ開く(毎回開閉) | 常置(開いたまま) |
|---|---|---|
| 版更新 1 回あたりの手数 | サーバ側で付与 → UI でアップロード → サーバ側で撤去(**手動配置——コピー 1 回——より多い**) | UI 操作のみで完結(最少) |
| 露出(プロセス侵害時に MSI 差し替えへ波及する期間) | 開けている間だけ(最小) | 常時(その代わり認証・監査・SACL・下記の通知で統制) |
| 向く環境 | 版更新が年数回・サーバ側操作(リモート PowerShell 可)が苦でない環境 | 更新頻度が高い・UI 完結を優先する環境(SACL 併用を推奨) |

**小規模環境(サーバ 1 台・管理者 1 人・版更新は年数回)の目安**: 手数だけを見るなら
手動配置(経路 (a))が最少であり、経路 (b) を毎回開閉で使う利点は薄い。経路 (b) が効くのは
**常置を選び、認証・監査・SACL で統制する**運用である。緊急の CVE 対応時、撤去済み運用では
まず付与操作が必要になる(手が増える)ことも考慮に入れること。

閉じ忘れへの備え: 機能を無効化したのに ACE が残っている状態は警告(イベント ID 1033)が、
機能有効のまま 24 時間以上開き続けている状態は情報レベルの定期通知(ID 1034。既定 7 日ごと)が
知らせる。生成画面にも「書き込み経路が開放されています」が常時表示される。

### アップロードの流れ

1. 生成画面(`/admin/forwarder-kit`)のアップロード欄でファイルを選ぶ(サイズ上限
   100 MiB——超過は送信前に画面で分かる)
2. 「アップロードして内容を確認」を押す。サーバがステージング領域(配置フォルダ内・
   検出には現れない一時名)へ受け取り、版(ProductVersion)・SHA256・公式配布 SHA256 との
   照合結果を表示する。**この時点ではまだ配置は確定していない**
3. 内容を確認して「配置を確定」を押す。既存 MSI がある場合は旧/新の SHA256 が並んで
   表示され、置き換えの確認チェックが必要になる。公式配布 SHA256 と一致しない場合
   (検証済み版以外はすべて該当)は、理解した上で進める確認チェックが必要になる
4. 確定すると検出結果に反映され、以後の生成で同梱を選べる。配置・削除・失敗はすべて
   監査記録(イベント ID 2026/2027/3014。操作者の利用者名つき)に残る

**格納ファイル名は Yagura が MSI の ProductVersion から生成する**(アップロード元の
ファイル名は使わない)ため、ファイル名の命名規則を意識する必要はない。MSI から版を
読み取れないファイル(破損・MSI 以外)は拒否される。
...
| `-Mode` | `udp` | 転送プロトコル(`udp` / `tcp`)。特性の違いは「UDP と TCP のどちらを使うか」を参照(Issue #156)。TLS 暗号化転送は本キットでは提供しない(後述「TLS 暗号化転送について」参照) |
...
- **管理 UI(`/admin/forwarder-kit`)で生成するキットも転送方式(UDP/TCP)を選択できる**
  (Issue #156)

### 選択の目安

- 大きなイベント(特に挿入文字列の多い Security 監査・詳細な失敗イベント)を
  **欠落なく**届けたい環境は `-Mode tcp` を検討する。ただし本文に
...
Yagura は syslog over TLS 受信(RFC 5425。TCP 6514 既定。opt-in——
[security.md](../design/security.md) §6)に対応しているが、**この Fluent Bit 配布キットからは
TLS 暗号化送信を提供しない**(オーナー決定 2026-07-11。security.md §6.1・
[ADR-0008](../adr/0008-forwarder-kit-generation.md) 改訂履歴 3)。

- **理由(2026-07-11 実機検証)**: RFC 5425 は octet-counting フレーミングのみを許容し、Yagura の
  TLS 受信はこれを厳格に強制する(先頭バイトが数字でなければ即座に接続を切断する)。ところが
  Fluent Bit の `out_syslog` は TCP 転送(TLS はその上に乗る)で **RFC 6587 の octet-counting を
  実装していない**(`-Mode tcp` の LF 区切り制約と同一原因。
  [`out_syslog` の実装](https://github.com/fluent/fluent-bit/blob/master/plugins/out_syslog/syslog.c)。
  ソース確認は 2026-07-09、実機検証は 2026-07-11)。**実機検証の結果(Fluent Bit 5.0.8 → Yagura
  TLS 受信)**: TLS ハンドシェイクは正常に成立するが、その直後、Yagura は LF 区切りのフレームを
  「octet-counting ではない = 回復不能なフレーミング破損」として接続ごと切断する。`out_syslog`
  は送達確認を待たない実装のため、**Fluent Bit 側はこの拒否を検知できず、リトライもエラー報告も
  行わない**(2 回の送信をそれぞれ別の keep-alive 接続で試行し、いずれも Yagura 側で拒否されたが
  Fluent Bit 側にはエラーが一切記録されなかったことをラボで確認した——送信側から見た**無音の
  喪失**)。「選ぶと無音で失う」組み合わせをキットが提供することは、この配布キットの設計思想
  (沈黙する失敗様式を作らない——ADR-0008)に反するため、キットの TLS 送信は導入しない
- **TLS 暗号化転送が必要な場合の代替**: octet-counting フレーミングに対応した送信実装を使う。
  代表例は **rsyslog** の `omfwd`(`Target` + `Protocol="tcp"` + `StreamDriver="gtls"` +
  RFC 5425 の octet-counting は rsyslog 側で `TCP_Framing="octet-counted"` 相当を指定)や
  **syslog-ng** の RFC 5425(`syslog()` ドライバ + `transport("tls")`)。Yagura の TLS 受信は
  これらの octet-counting 対応実装からは正常に受信できる(本 PR では openssl s_client による
  受信側の実機検証を実施済み。security.md §6.1・SEC-11)
- **再導入の見込み**: Fluent Bit の `out_syslog` が octet-counting に対応した時点で、本キットへの
  TLS 送信の再導入を再評価する(security.md §6.1 の再評価トリガ)
- **Yagura 側の TLS 受信の設定・挙動**(証明書の参照方式・期限切れ時に受信を止めない設計・
  サーバ認証のみ相互 TLS 非対応・証明書の周期監視通知)は [security.md](../design/security.md)
  §6・§6.1 を参照
...
- **送信**: RFC 5424 / UDP・TCP(いずれも平文)から `-Mode` で選択(既定 UDP、特性は前述
  「UDP と TCP のどちらを使うか」参照。TLS 暗号化送信は本キットでは提供しない——前述
  「TLS 暗号化転送について」)。HOSTNAME = イベントの `Computer`、APP-NAME = `ProviderName`
...
- **TLS 暗号化転送(syslog over TLS。Issue #137)は本キットでは提供しない**(前述「TLS 暗号化
  転送について」)。Fluent Bit の `out_syslog` が octet-counting 非対応のため Yagura の TLS 受信の
  送信元にできないことを 2026-07-11 に実機確認した(Fluent Bit 5.0.8 → Yagura。TLS ハンドシェイクは
  成立するが後続メッセージが全て拒否され、Fluent Bit 側は無音で失う)。Yagura の TLS 受信側の
  単体動作は `openssl s_client` を送信元として検証済み——①期限内証明書での TLS ハンドシェイク +
  octet-counting フレーミング送信が Yagura に到達・解析・保存されること、②Yagura に既に期限切れの
  証明書を構成しても TLS 受信リスナが起動・接続受理を継続すること(「止めない」設計)、③期限切れ
  証明書に対する厳格な証明書検証(`-verify_return_error`)がクライアント側でハンドシェイク拒否を
  起こし、Yagura 側はこれをフレーミング失敗とは別経路の「TLS ハンドシェイク失敗」として記録する
  こと、④証明書の期限接近・期限切れの周期監視通知(EventId 1017/1018)が実プロセスで正しく
