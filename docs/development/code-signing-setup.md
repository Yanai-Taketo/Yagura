...
>
> - API: `iamcredentials.googleapis.com`・`cloudkms.googleapis.com` を有効化済み
...
# 2-0) 必要な API を有効化する。
# iamcredentials は「WIF のトークンでサービスアカウントを借用する」段で初めて要るため、
# 2-1〜2-4 の作成・確認がすべて成功しても、実際に CI から署名しようとして初めて
# SERVICE_DISABLED で失敗する(2026-07-18 に実機で遭遇)。ここで先に有効化しておく。
gcloud services enable iamcredentials.googleapis.com cloudkms.googleapis.com
