...
コメントは「コードから読み取れない **why**」だけを書く。決定の経緯・出典は git 履歴・ADR・PR が唯一の正であり、コメントに複製すると改訂に追従できず嘘になる(「依存バージョンのコメント規約」と同じ原理)。

- **書くもの**:
  - 実装選択の理由・トレードオフ・却下した代替案の要点(コードだけでは読めないもの)
  - 不変条件・並行性/整合性の落とし穴・境界条件・既知の限界(known limitation)
  - 既定値や定数の「意味」(なぜこの値か、変えると何が壊れるか)
- **書かないもの**:
  - **経緯・出自の語り**: PR 番号、レビュー指摘への言及、レビュアー・ペルソナの人名、「オーナー決定 YYYY-MM-DD」等の裁定日付、採番・命名の経緯
  - **変更履歴の語り**: 「以前は〜だった」「のち ADR-NNNN で撤去した」「v1→v2→v3」等、過去の状態や撤去済み機能の説明(git log と ADR の改訂履歴で追える)
  - 次の行がやることをなぞるだけの説明
- **出典参照は識別子 1 つまで**: `（ADR-0010 決定 3）` `（architecture.md §4.2）` のように最も関係の深い 1 参照に留める。複数 ADR・決定番号の連結はしない。詳細説明は参照先の文書に置き、コード側では要点を自分の言葉で 1〜3 行に言い切る
- **XML doc の `<remarks>` は設計書の複製にしない**: 型の使い方に必要な契約(スレッド安全性・呼び出し順序・破棄規約)に絞る。設計の背景解説が長くなるなら docs/design/ 側へ移し、参照 1 行を残す
- 本規約は src / tests の両方に適用する
...
- **「環境変更への追従」名目の小さな引き下げの積み重ねを禁止する**: 性能公称値（M-6）確定後は、
  ...
  （オーナー承認 + 公開の場での記録）で扱う。なお **M-6 の基準環境は ADR-0024 決定 1 の推奨区分
  （Windows Server 2022 / 2025）から選ぶ**制約があり、公称値の表明対象も推奨区分に限られる
  （architecture.md §5.2）
