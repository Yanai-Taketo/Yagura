...
| 動作基盤 | 専用アプライアンス等 | Linux | Windows ネイティブ（.NET・単一の Windows サービス） |
...
解析に失敗したメッセージも破棄せず、受信した生データのまま「解析失敗」の印を付けて保存します。取りこぼしは発生箇所別のカウンタで観測できます。プロセス内の境界・信頼性機構・受信パイプラインの詳細は [docs/design/architecture.md](docs/design/architecture.md) を参照してください。
