...
- **syslog 受信**: UDP 514 / TCP 514（平文・IPv4/IPv6 両対応）。RFC 3164 / RFC 5424 の両形式を解析します。syslog over TLS（TCP 6514・RFC 5425）を opt-in で有効化でき、証明書は期限切れ・失効時も受信を止めずに提示を継続します（サーバ認証のみ）。証明書は管理画面から Windows 証明書ストアの一覧で選択できます（設定ファイルの手編集は不要）
- **Windows イベントログの転送**: Fluent Bit（Apache-2.0）の配布キットを同梱し、Windows 端末のイベントログを syslog（RFC 5424）として集約できます。サイレント導入に対応し、管理画面の「フォワーダキット生成」で自機宛の設定入りキット（ZIP）を生成できます（[手順書](docs/guides/forward-windows-eventlog.md)）。同梱用 MSI の配置は、管理 UI 認証を有効化した構成なら管理画面からのアップロード（opt-in）でも行えます（[ADR-0020](docs/adr/0020-forwarder-msi-upload.md)）
...
- **能動通知のメール送信（opt-in）**: 運用者の対応を要する警告・エラー（証明書の期限接近・スプール退避の開始・DB への書き込みの恒久失敗など）を SMTP でメール送信できます。既定は無効です。管理画面で SMTP サーバ・宛先を設定し、テスト送信で疎通を確かめられます（SMTP パスワードは Windows DPAPI で暗号化保存）。同一事象の再送抑制と全体の流量上限を備え、通知メール自身の障害はメールの対象にしない設計でループを排除しています。イベントログへの記録は従来どおり常時行われます
...
- **閲覧 UI の HTTPS（opt-in）**: 閲覧 UI を HTTPS で公開できます。ポートはそのまま（既定 8514）で平文 HTTP から HTTPS に切り替わり、平文の面は残りません。証明書は管理画面から Windows 証明書ストアの一覧で選択でき、サーバ名と証明書の名前（SAN）の不一致は保存前に検査して「警告なしでアクセスできる URL」を具体的に提示します。期限切れ・失効時は平文へ落とさず HTTPS を停止し（syslog 受信・管理画面は影響を受けません）、期限の接近は事前に警告します（[ADR-0022](docs/adr/0022-viewer-https.md)・[運用ガイド](docs/operations.md)）
...
MSI には Authenticode コード署名（タイムスタンプ付き）を付与しています。署名者（発行元）は開発者個人の実名「**柳井建人**（YANAI Taketo）」です。仕組み・体制・詳細な検証手順は[コード署名ポリシー](docs/code-signing-policy.md)を参照してください。

- 警告画面や UAC で発行元が実名で表示されたら、**「柳井建人」と一致することを確認したうえで**実行してください。**一致しない場合は実行しないでください**
- 署名の実績が SmartScreen に蓄積されるまでの間は、署名済みでも「Windows によって PC が保護されました」と表示されることがあります。その場合も「詳細情報」で発行元を確認してから進めてください
- **SmartScreen の青い画面では発行元の日本語名が文字化けして見えることがあります**（例: `���縺�` のような表示）。これは Windows 側の既知の表示問題（非 ASCII の発行元名を ANSI で誤デコードする）で、署名自体の異常ではありません。**正確な発行元は MSI のプロパティ →「デジタル署名」タブ、または UAC の昇格ダイアログで確認してください**（こちらは正しく「柳井建人」と表示されます）
- あわせて、ダウンロードした MSI のハッシュを同じ Release に添付の `.sha256` と照合できます（PowerShell で `Get-FileHash <ファイル> -Algorithm SHA256`）

署名を付与していない過去のリリース（v0.4.0 以前）では発行元が「不明」と表示されます。

### システム要件

**まず試すのに必要なものは、対応 OS とブラウザだけです。** データベース製品の準備は要りません（SQLite を同梱しています）。SQL Server は「本番昇格」を選ぶときに初めて必要になります。

#### 1. OS とアーキテクチャ

自分の環境が当てはまるかは、**①アーキ → ②OS のバージョン**の順で確かめてください。

**①アーキ**: 「設定 > システム > バージョン情報」の「システムの種類」を見ます。「x64 ベース プロセッサ」なら x64 版、「ARM ベース プロセッサ」なら ARM64 版の MSI です。**迷ったら x64 です**——通常の Windows PC・Windows Server はほぼ x64 です。

**②OS のバージョン**: `Win` + `R` で `winver` を実行すると「バージョン ○○ (OS ビルド ○○○○○)」が表示されます。

| 区分 | 対象 | 補足 |
|---|---|---|
| **推奨** | Windows Server 2022 / 2025（x64） | **これに当てはまるなら追加作業はありません**（ブラウザも同梱） |
| **対応** | Windows Server 2019（x64） | 実機検証済み（2026-08-02）。**この OS には TLS 1.3 がありません**——TLS を使う機能は 1.2 での通信になります（実機で確認しました） |
| **対応** | Windows 11 / Windows 10 の LTSC・Enterprise（x64。**OS ビルド 17763 以上**） | 試用・小規模用途向け |
| **試験的** | Windows 11 on Arm 等のクライアント（ARM64） | 不具合報告は受け付けますが修正時期は約束しません（[ADR-0009](docs/adr/0009-architecture-support.md)） |
| **対応外** | Windows Server 2016 以前 / Server Core・Nano Server / **Windows 10 の Home・Pro** / 32-bit | 下記参照 |

**対応外の理由**（詳細は [ADR-0024](docs/adr/0024-supported-environments.md)）:

- **Windows Server 2016 以前**: 延長サポートが 2027-01-12 に終了するため
- **Windows 10 の Home・Pro**: **2025-10-14 に提供終了済み**であり、かつ .NET 10 のサポート対象が LTSC・Enterprise に限られるため（Windows 11 と Windows 10 LTSC/Enterprise は対応します）
- **Server Core・Nano Server**: 動作しないと判明したわけではなく、**検証していない**ためです。設定ファイルの手編集による管理経路は設計上存在します（[ADR-0004](docs/adr/0004-security-model.md) 決定 2）
- 対応外の環境をお使いで Yagura に関心がある場合は、[Discussions](https://github.com/Yanai-Taketo/Yagura/discussions) でご相談ください

なお、Windows イベントログの転送元となる収集対象端末（Fluent Bit を導入する側）のアーキ対応は、Yagura サーバ本体のアーキとは独立の話題です。x64・ARM64 の両方に対応しています（[転送ガイド](docs/guides/forward-windows-eventlog.md)）。

#### 2. ブラウザ

**最新版の Microsoft Edge / Google Chrome / Mozilla Firefox / Apple Safari** のいずれか。Internet Explorer には対応していません。

- **日々のログ閲覧は、手元の PC など任意の端末のブラウザで行えます**（サーバにブラウザを常用する必要はありません）
- **サーバ機自身にブラウザが必要になるのは、初回セットアップと設定変更のときです**。管理画面が既定で loopback 固定のためです（`http://localhost:8515/admin`）
- Windows Server 2022 以降は Edge が同梱されているため、そのまま使えます。**Windows Server 2019 では Edge が同梱されないため、MSI を実行する前に対応ブラウザを導入してください**
- インターネットに接続しないサーバへ導入する場合は、オフラインインストーラを別途入手してください（Yagura を閉じたネットワークに置くこと自体は推奨構成です。下記「セキュリティの前提」参照）

#### 3. 保存先データベース（本番昇格を行う場合のみ）
...
**CPU とメモリの下限は、実測にもとづく確定作業の途中です。** 根拠のない数値を要件として書かない方針のため、確定するまで記載しません（[ADR-0024](docs/adr/0024-supported-environments.md) 決定 4）。

ディスクは、**まず既定の構成（SQLite・保持 30 日）で 1 週間ほど動かし、「システム状態」画面に出る実際の受信量を次の式に入れて**見積もってください。

```
必要なディスク ≒ (平均メッセージ長 + 95 B) × 秒間の受信件数 × 保持期間の秒数
                 + スプールの使用量上限 + 空き容量の余裕（1 GiB 以上）
```

この式は SQLite を保存先にした場合の実測にもとづきます。**SQL Server は文字列の格納方式が異なるため、この式より大きくなります**（確定は実測待ちです）。

> 参考までに、性能の上限を測るための構成（8 論理コア / 7 GiB・SQL Server Express 同居）での実測値は毎秒 4,500〜5,000 件です。**これは推奨構成ではありません**——ネットワーク機器数十台の一般的な環境では毎秒 10 件程度のことが多く、その規模でこの構成は必要ありません。

#### 5. サポート期間について

**v1.0 系列のサポートは 2028-11-14 までです**（同梱している .NET 10 のサポート終了日）。**対応 OS のサポート期限より先に Yagura のサポートが終わります**（例: Windows Server 2019 は 2029-01-09、Windows Server 2022 は 2031-10-14）。修正の提供条件は [SECURITY.md](SECURITY.md) を参照してください。

また、**Windows Server 2019 と SQL Server 2019 は Microsoft のメインストリームサポートが終了済み**です（それぞれ 2024-01-09 / 2025-02-28）。以降これらに提供されるのはセキュリティ更新のみで、機能不具合の修正は出ません。Yagura 側でも、これらの環境に固有の不具合は推奨環境の品質維持を優先して対応順位を判断します。**新規に構築するなら Windows Server 2022 以降 + SQL Server 2022 以降をお勧めします。**
...
| TCP 6514 | syslog over TLS 受信（暗号化。RFC 5425。既定無効の opt-in） | 全インターフェース・IPv4/IPv6（設定で縮小可） |
| TCP 8514 | 閲覧 UI（読み取りのみ） | LAN（設定で localhost のみに縮小可。opt-in で Windows 統合認証 + AD グループ認証） |
...
- 閲覧 UI の HTTPS は **opt-in** です（既定は平文 HTTP。有効化すると同一ポートのまま HTTPS に切り替わり、証明書の期限切れ・失効時は平文へ落とさず HTTPS を停止します——[ADR-0022](docs/adr/0022-viewer-https.md)）。閲覧認証（特に `AdminGroups`）を使う構成では HTTPS の併用を推奨します
- ただし**書き込み系の管理操作（設定変更・DB 切替・保持期間変更等）は、既定でサーバ自身（localhost）からのみ実行できます**。LAN 上の第三者が設定を書き換えることはできません
- MSI が作成する受信許可規則（UDP/TCP 514・閲覧 8514）は**ネットワークプロファイルが Domain + Private 限定**です。Public プロファイル（公衆ネットワーク等）には開放されません。**TLS 受信（6514）を後から有効化した場合、その規則は自動では追加されません**（手順は[運用ガイド](docs/operations.md)を参照。規則が無い間は起動時に警告が出ます）
- **TLS の通信強度は OS 側の設定に依存します。** Yagura が固定しているのは「TLS 1.2 以上」というプロトコルの下限だけで、**実際に使われる暗号スイートの選択は Windows（Schannel）に委ねています**。Windows の既定では **SHA-1 の CBC スイート**が有効なままのため、それしか使えない相手とはその強度で通信が成立します（**3DES** は Windows Server 2019・2022 が該当。Server 2025 では既定から外れています）。強度を統制したい場合は OS 側のハードニング（グループポリシー等）で不要なスイートを無効化してください
  - この点で **Windows Server 2019 と 2022 の既定は同一**です（実測。[ADR-0024](docs/adr/0024-supported-environments.md) 改訂 8）。「対応環境は推奨環境より暗号が弱い」ということはありません
