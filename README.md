...
**v0.3 は評価・試用向けのリリースです。本番利用はまだ推奨していません。**
...
- **syslog 受信**: UDP 514 / TCP 514（平文・IPv4/IPv6 両対応）。RFC 3164 / RFC 5424 の両形式を解析します。syslog over TLS（TCP 6514・RFC 5425）を opt-in で有効化でき、証明書は期限切れ・失効時も受信を止めずに提示を継続します（サーバ認証のみ）
...
「ログを失わない設計」（[できること（v0.3）](#できることv03)を参照）は、このパイプラインの各段に組み込まれています。プロセス内の境界・信頼性機構・受信パイプラインの詳細は [docs/design/architecture.md](docs/design/architecture.md) を参照してください。
...
v0.3 の既定構成は、**管理された社内ネットワーク（信頼ネットワーク）での利用を前提**としています。
...
- 管理 UI は既定で **loopback 限定・無認証**ですが、opt-in で **Windows 統合認証（AD/Kerberos）またはアプリ独自 ID/パスワード認証**を有効化でき、リモート公開する場合は **HTTPS 必須（fail-closed）** です（[ADR-0010](docs/adr/0010-admin-ui-authentication.md)）。**閲覧 UI 側の認証・HTTPS はまだ提供しておらず**、v1.0 までに opt-in での提供を検討します
