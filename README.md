...
- **TLS の通信強度は OS 側の設定に依存します。** Yagura が固定しているのは「TLS 1.2 以上」というプロトコルの下限だけで、**実際に使われる暗号スイートの選択は Windows（Schannel）に委ねています**。OS の既定で古いスイート（3DES・SHA-1 系・NULL 等）が有効なままだと、それを提示してくる相手とはその強度で通信が成立します。強度を統制したい場合は OS 側のハードニング（グループポリシー等）で不要なスイートを無効化してください
