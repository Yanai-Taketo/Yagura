...
本プロジェクトは、オーナー（YANAI Taketo）の承認を最終ゲートとして、AI エージェント（Claude）と協働で開発しています。コミットや PR にある `Co-Authored-By: Claude ...` の表記はその記録です。設計判断の経緯はすべて PR 上のコメントと ADR に残しています。
