...
/// </summary>
/// <remarks>
/// 接続を開いて <c>SELECT 1</c> を実行する最小の検証。スキーマ・権限の事前検証
/// （database.md §5.2 の SQL 提示・§6.1 準備フェーズの「接続・権限・スキーマの事前検証」の
/// 完全形）は切替本番の実行時手順と合わせて後続 Issue で拡充する——本骨格は「接続の成立」
/// のみを検証の対象とする。
/// </remarks>
public sealed class SqlServerConnectionValidator : ISqlServerConnectionValidator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex) when (ex is SqlException or InvalidOperationException or ArgumentException or FormatException)
        {
            ...
            // して利用者向けにそのまま返す（監査記録には載せない——記録するのは成否のみ。
            // PromotionWizardService 参照）。
            return new SqlServerConnectionValidationResult(false, $"接続できませんでした: {ex.Message}");
        }
    }
}
