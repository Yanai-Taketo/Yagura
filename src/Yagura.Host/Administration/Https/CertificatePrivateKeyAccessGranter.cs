...
/// <summary>
/// 証明書の秘密鍵読み取り権限を、サービスアカウントへ付与する。管理リスナのリモート HTTPS
/// （ADR-0010 Phase 2 決定 4）と TLS 受信（ADR-0019）の双方の証明書に用いる共有実装
/// （型名は特定用途に寄せず中立とする）。configuration.md §6 が確定済みの方式（証明書の選択時に、
