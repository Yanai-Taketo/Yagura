...
public static class AdminAuthenticationDefaults
{
    ...
    /// <summary>
    /// バックオフ上限 cap（仮値: 30 秒。ADR-0011 決定 3）。cap に達しても拒否はしない——
    ...
    /// <summary>
    /// IP レート制限の窓（仮値: 60 秒。ADR-0011 決定 4）。loopback（<c>127.0.0.1</c>・
    /// <c>::1</c>）は明示除外——判定点は
    ...
    /// <summary>
    /// グローバルトークンバケットのバースト容量（仮値: 20。ADR-0011 決定 5.1。Phase 2 の
    /// 複数オペレータ同時対応を踏まえた値）。loopback は明示除外。
    /// </summary>
    public static readonly int GlobalBucketBurst = 20;

    /// <summary>
    /// IP レート制限・グローバルトークンバケット層の <c>Retry-After</c> 上限（仮値: 30 秒。
    /// バックオフ cap と揃える。ADR-0011 決定 5.1）。
    /// </summary>
    ...
    /// <summary>パスワード最小長（確定値: 12 文字以上。ADR-0011 決定 7）。</summary>
