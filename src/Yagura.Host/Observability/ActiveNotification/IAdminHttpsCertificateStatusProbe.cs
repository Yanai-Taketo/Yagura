...
/// <summary>
/// 管理リスナのリモート HTTPS 証明書（ADR-0010 Phase 2 決定 4）の現在の状態を照会する契約
/// （PR #224 レビュー指摘 #2・#3 への対応）。
/// </summary>
/// <remarks>
/// <para>
/// <b>なぜ抽象化するか</b>: 実装（証明書ストアの照会——
/// <c>Yagura.Host.Administration.Https.AdminCertificateProvider</c>）は Windows 専用 API
...
/// </para>
/// </remarks>
public interface IAdminHttpsCertificateStatusProbe
{
    /// <summary>
    /// 証明書の現在の状態を照会する。呼び出しは <see cref="ActiveNotificationMonitor"/> の
    /// 周期（<see cref="ActiveNotificationConstants.PollInterval"/>）ごとに 1 回。
    /// </summary>
    AdminHttpsCertificateStatus Check();
}

/// <summary>
/// <see cref="IAdminHttpsCertificateStatusProbe.Check"/> の結果。
/// </summary>
...
/// 期限接近（<see cref="ActiveNotificationConstants.AdminHttpsCertificateExpiryWarningWindow"/>）・
/// 期限切れの判定入力。
/// </param>
...
/// 秘密情報は含まない）。
/// </param>
public sealed record AdminHttpsCertificateStatus(
    bool IsAvailable,
