...
/// 「1000 番台 = 運用警告」）に割り当てるイベント ID（SEC-5 の 1000 番台側）。
/// </summary>
/// <remarks>
/// <para>
...
public static class ActiveNotificationEventIds
{
    ...
    /// `Spool:Directory` が別ドライブに向いた構成でも「夜間にスプールが
    /// 満ちていく」現場のボリュームを見逃さないため）。レベル: 警告。
    /// </summary>
    ...
    /// 監視自身が無警告で沈黙・停止する経路を残さない）。レベル: エラー
    ...
    /// <summary>
    /// スプールの定期自己検証（architecture.md §3.2.5）が失敗した——合成レコードの
    ...
    /// （経路障害が疑われる。バックログ起因（<see cref="SpoolSelfTestTimeoutBacklog"/>）との判別の
    /// 閾値は <see cref="ActiveNotificationConstants.SelfTestTimeout"/>）。レベル: エラー
    ...
    /// <summary>
    /// スプールの定期自己検証（architecture.md §3.2.5）がタイムアウトしたが、同じ
    ...
    /// マーカーの追跡が終わるまで本 ID へ戻さないラッチ（振動防止）は
    ...
    /// Phase 2 決定 4——起動時警告 1013 だけでは稼働中の異常が無音に
    ...
    /// <summary>
    /// TLS 受信（<c>Ingestion:Tls:Enabled</c>。RFC 5425。opt-in。security.md §6）の
    ...
    /// 以上連続して観測された（ADR-0020 決定 2）。設定上は期待どおりの状態で
    ...
    /// 状態の可視化のため。「使うときだけ開く」運用の閉じ忘れの主たる検出手段）。
    /// </summary>
    ...
    /// 流用——用途別の値を採用すべき設計上の根拠が無い）。閲覧 HTTPS は
    ...
    /// （ADR-0023 決定 1）。受信・閲覧・Windows 統合認証は継続する。
