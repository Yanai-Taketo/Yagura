...
public sealed class TcpSyslogListenerOptions
{
    ...
    /// ソケットで bind し、IPv4・IPv6 の両方を単一ソケットで受信する（<see cref="TcpSyslogListener"/> の
    /// remarks・<see cref="Yagura.Ingestion.Net.DualStackBindAddress"/>
    ...
    /// 暫定採用する（M-14 の同時接続数上限の実測確定と合わせて再評価する候補——
    /// architecture.md §9 の実測待ち一覧には未掲載のため、暫定値であることをここに明示する）。
    /// </remarks>
    public static readonly TimeSpan DefaultIdleTimeout = TimeSpan.FromMinutes(5);

    /// <summary>
    /// フレーミング進捗タイムアウトの既定値（仮値。実利用で確定する）。
    /// </summary>
    /// <remarks>
    ...
    /// と同一（IPv6 不可時: 既定は IPv4 へ自動縮小 / 明示は fail-fast）。
    /// </summary>
    ...
    /// 最後にバイトを読み取ってからこの時間が経過すると接続を切断する。
