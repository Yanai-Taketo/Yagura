...
public sealed class TcpFrameDecoderOptions
{
    ...
    /// </summary>
    /// <remarks>
    /// 有効なメッセージが 1 件も確定しないまま読み捨てたバイト数（フレーム間 LF/CR のスキップ・上限超過メッセージ本体の
    /// 読み飛ばし・non-transparent-framing の破棄行）がこの上限を超えた接続は、再同期の見込みが
    /// ないゴミデータ送信元と判断して切断する。他社実装の調査では、フレーミング
    /// エラー即切断（rsyslog・syslog-ng・Fluent Bit 等）が業界主流であり、この寛容化は
    /// 一次防御を外した状態になる——本上限は寛容化と組み合わせて同等の防御水準を保つ天井
    /// である。既定値 128 KiB は、既定の 1 メッセージ上限（<see cref="DefaultMaxMessageLength"/>
    /// = 64 KiB）を超えたメッセージ 1 通分の読み飛ばし + 余裕を許容しつつ
    ...
    /// </summary>
    /// <remarks>
    /// RFC 5425（syslog over TLS）§4.3 は
