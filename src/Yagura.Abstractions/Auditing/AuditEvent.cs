...
/// <param name="Detail">
/// 事象の要約（security.md §4.1「何を・変更前後の要約」）。
...
/// 認証に使われた方式（ADR-0010 決定 6。<c>"windows"</c> / <c>"app"</c>。認証 opt-in が
...
/// 認証済み利用者名（ADR-0010 決定 6 の「命名空間つき表記」の名前部分。Windows 統合認証は
...
/// <see cref="AuthenticationScheme"/>・<see cref="AuthenticatedPrincipal"/> は省略時
/// <see langword="null"/>（未認証操作を表す）。
/// </remarks>
